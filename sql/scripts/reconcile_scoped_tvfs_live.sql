/*───────────────────────────────────────────────────────────────────────────────────────────
  reconcile_scoped_tvfs_live.sql   —   ONE-TIME LIVE RECONCILE (run once against Assessment_Warehouse)

  WHY: the 2026-09-22 'scope RegionalAnalyst by StaffSchoolAccess' security fix (13 TVFs) landed on
  LIVE only partially — some TVFs got the new scoping, others kept the old definition that returns
  ZERO for an analyst. Symptoms: Reports 'No students in your scope', Programming chips 'No IPPs /
  No Adaptations on record' + empty Programming roster, while the group CARDS (tvf_ProgrammingGroups,
  already current) correctly count records. Dev is fine; this is a live-deploy drift only.

  WHAT: redeploys every @UPN-scoped security TVF from current source. Each is DROP-IF-EXISTS +
  CREATE + GRANT (idempotent) — a TVF already current just gets rewritten to the identical body.
  Dependencies (SectionRosterMembership, FactStudentAdaptation, vw_StudentReadingStartingPoint,
  etc.) are already on live from the 0.6.2 cutover + prior deploys.

  Generated 2026-09-24 from HEAD 311a2ff. Regenerate if any TVF changes.
───────────────────────────────────────────────────────────────────────────────────────────*/
...
-- ============================================================================
-- tvf_StudentAssessmentHistory.sql
-- ============================================================================
/*******************************************************************************
...
-- ============================================================================
-- tvf_StudentAssessmentHistoryWriting.sql
-- ============================================================================
/*******************************************************************************
...
(
    ...
    SELECT DISTINCT
        ...
        -- Achievement band (delta -> colour/name) is computed CLIENT-SIDE in RosterEntry (it must
        -- update live as the teacher picks a level), so the old DimAchievementLevel join + its
        -- columns were dead server work and were removed 2026-09-23.
