 ...
 *       ReadingScaleID:     ThisItem.NewLevel.ReadingScaleID,
 *       AssessmentDate:     Today()
 *   })
 *
 * Parameters:
 *   @StudentNumber      BIGINT  — required, provincial 10-digit student #
 *   @AssessmentWindowID BIGINT  — required, must resolve to ActiveFlag=1
 *   @ReadingScaleID     BIGINT  — required, must resolve to ActiveFlag=1
 *                                 and ScaleSystem must match window's
 *   @AssessmentDate     DATE    — required (used for INSERT path's
 *                                 effective-date StudentKey resolution and
 *                                 stored on the fact row; IGNORED on UPDATE)
 *
 * Server-resolved:
...
CREATE PROCEDURE usp_UpsertReadingAssessment
    ...
    @AssessmentWindowID BIGINT,
    @ReadingScaleID     BIGINT,
    @AssessmentDate     DATE
AS
BEGIN
    SET NOCOUNT ON;
    ...
    WHERE ReadingScaleID = @ReadingScaleID
      AND ActiveFlag = 1;
    ...
    WHERE StudentKey = @StudentKey
      AND AssessmentWindowID = @AssessmentWindowID;

    IF @ExistingAssessmentID IS NOT NULL
    BEGIN
        UPDATE FactAssessmentReading
        SET ReadingScaleID      = @ReadingScaleID,
    ...
    VALUES (
        ...
        CONCAT(
            ...
            ' | AssessmentWindowID=', CAST(@AssessmentWindowID AS VARCHAR(20)),
            ' | ReadingScaleID=',     CAST(@ReadingScaleID     AS VARCHAR(20)),
