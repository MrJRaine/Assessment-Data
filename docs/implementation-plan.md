...
- [ ] **B4. Fabric write** — call an existing wrapper proc (e.g. `usp_InsertSubmissionAudit`) from the app; confirm the row lands with the caller's UPN. (Connection + `@azure/identity` token path now proven by B3; needs `GRANT EXECUTE` from `sql/security/grant_webapp_sp.sql` and a write call.)
