...
### Left Off — 2026-06-19 (B3 FABRIC READ PROVEN — pure-JS @azure/identity token + mssql 12 / tedious 19; the whole "tedious can't do Fabric" saga = "tedious 18 can't, 19 can")
- **Last completed**: **B3 — the containerized app reads `Assessment_Warehouse` end-to-end** (`/api/dbcheck` → `{ok:true, connected_as:"StudentDataAssessment"}`). Solved a multi-step `ESOCKET "Connection lost - socket hang up"` failure that survived three driver theories.
- **Root cause + fix** (full detail in memory [[project_webapp_fabric_connection]]): (1) tedious's *own* `azure-active-directory-service-principal-secret` flow mints a token Fabric rejects → mint it ourselves with `@azure/identity` `ClientSecretCredential` (`https://database.windows.net/.default`) and pass via `azure-active-directory-access-token`; (2) **`tedious` 18 cannot complete Fabric's TDS login, `tedious` 19 can** (PR #1668) — so **`mssql@^12`** (uses tedious 19), NOT `mssql@^11` (pins tedious 18); (3) Next standalone does NOT trace `tedious` (mssql loads it via a *dynamic* require) → keep the DB stack in `serverExternalPackages` AND ship `node_modules` into the runner (Dockerfile `proddeps` overlay). Explored ODBC/`msnodesqlv8` first (per a Reddit thread) — it segfaults on Linux AAD; abandoned for the pure-JS path. Image stays Alpine, 583 MB.
- **Versioning milestone**: tagged `webapp-v0.1-tedious-baseline` (commit `55f7627`) before the swap; recover with `git checkout webapp-v0.1-tedious-baseline`.
- **Next action**: USER — still (1) **send the bundled web-app IT request** (the SP/grants used here are on the *shared dev* SP; production needs the IT-gated app reg) and (2) open the PR. CLAUDE — **B4 write** (run `sql/security/grant_webapp_sp.sql`, then call a wrapper proc from the app), then **B5** (bind a real secured-view read into the window/group screen via `queryAsUser`). Remove the temporary `/api/dbcheck` probe before any deploy.
- **Note**: `webapp/.env` holds the working SP creds + Fabric server; never commit it. The SP display name in the warehouse is `StudentDataAssessment`.
