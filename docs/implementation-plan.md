...
- [x] **B2. Entra login — access control + ID validation** — **DONE 2026-06-19** (confirmed against the TCRCE tenant: silent SSO + fresh-browser interactive login through MFA both land signed-in; app reg `c33fb2d3`; `AUTH_URL` set so the container emits the `localhost` redirect_uri not `0.0.0.0`; learned the portal Secret *ID* ≠ Secret *Value* — only the Value goes in `ENTRA_CLIENT_SECRET`). Auth.js (NextAuth v5) + Microsoft Entra ID provider; sign-in/out, session, UPN lifted onto the session (`src/auth.ts`, `src/lib/auth.ts`, landing-page access-control panel). Verified under Podman: Auth.js initializes, `/api/auth/session` → `null`, sign-in button renders, no errors. Needs a single-tenant Entra app registration; redirect `http://localhost:3000/api/auth/callback/microsoft-entra-id`; delegated `openid/profile/email/User.Read` (user-consentable, no admin consent).
- [x] **B3. Fabric read** — **CONNECTION PROVEN 2026-06-19**. The containerized app reads `Assessment_Warehouse` end-to-end (`/api/dbcheck` → `{ok:true, connected_as:"StudentDataAssessment"}`). Root cause of the long `ESOCKET "Connection lost - socket hang up"` saga: **`tedious` 18 cannot complete Fabric's TDS login; `tedious` 19 can** (the fix in tedious PR #1668 shipped in 19.x). `mssql@11` pins `tedious@^18` (broken); **`mssql@12` uses `tedious@^19` (works)**. Also: do NOT use tedious's built-in `azure-active-directory-service-principal-secret` (Fabric rejects the token it mints) — instead mint the token in Node via `@azure/identity` `ClientSecretCredential` for `https://database.windows.net/.default` and pass it with `azure-active-directory-access-token`. See memory `project_webapp_fabric_connection`. STILL TODO: bind the real `queryAsUser`/secured-view read into a screen (that is B5); remove the temporary `/api/dbcheck` probe before deploy.
- [x] **B4. Fabric write** — **PROVEN 2026-06-19**. Called `usp_InsertSubmissionAudit` through the SP connection (`RecordType='Test'`, `Source='system'`, `Status='Test'`); the row landed in `FactSubmissionAudit` and read back (AuditID is a 19-digit BIGINT → cast surrogate keys to VARCHAR in any app-facing read). Confirms `GRANT EXECUTE` is in place AND ownership chaining works (SP has EXECUTE-only, no table DML, write still succeeds). Write primitive added to the data layer: `execProc(procName, inputs)` in `webapp/src/lib/db.ts`. (One `Test`/`system` audit row remains as evidence — harmless; remove via warehouse access if desired.)
...
- [x] **B5. Port first real screen** (window/group select) — **DONE + VERIFIED WITH DATA 2026-06-19.** Bridge views deployed; ran the container as `classroom.teacher1@tcrce.ca` → `/enter` rendered 2 real window cards (English + FR Immersion Elementary, `entered/applicable` counts) and the group drill-down rendered "Homeroom 6A" (label derived from `GroupKey` HR:6A). `@UPN` scoping confirmed (only that teacher's windows/groups). The two `/enter` screens now read live Fabric (no mock): `getCurrentUpn` → `getTeacherWindows` / `getTeacherGroups` (`webapp/src/lib/data.ts`) → `queryAsUser` with `@UPN` (+`@WindowID`) against `vw_BridgeTeacherRosterAll`, rendered as window/group cards with `entered/applicable` counts, plus empty + error states. Verified the wiring end-to-end in the container (page renders, query runs, error caught gracefully). **Reads bridge (RLS-stripped) views scoped by `@UPN` in code** — the documented service-principal model; scoping is enforced in `data.ts`, never exposed raw. BLOCKED on: (1) USER deploy `sql/security/bridge_views.sql` to the warehouse (SP has Read only, can't CREATE; the SP reads them via its existing ReadData); (2) a real teacher UPN with roster data to set `DEV_FAKE_UPN` for happy-path verification. Caller-scoped views stay for the future user-token/OBO path.
...
- [ ] **31. Add `FactAssessmentWriting`** and configure writing rubric entry *(now in the **web app**, not Power Apps)*. **Status (2026-06-25)**: SQL DONE + dev-proven — 4-trait 1-4 rubric, monthly windows, multiple dated entries, late entry, avg→band; `usp_UpsertWritingAssessment` + `tvf_*Writing` (cohort/history/roster) + DQ checks. **Remaining**: the web UI (4-input roster grid for Writing windows + Reading|Writing toggle on cohort/detail) and the live deploy. See [[project_ongoing_assessment_model]] + the latest Left Off.
...
- **Last completed**: **0.5.0** (Math P–6, Programming makeover, dual-language literacy + per-cycle scoping,
  ...
  streaming) both LIVE on data.tcrce.ca. **PR #32 merged to `main`; `v0.5.0` + `v0.5.1` tags pushed; `dev`
  synced (== main @ 3bb0c98).** Live SQL deployed via the staged idempotent bundles `sql/deploy/live_0.5.0/`
  (generated by `scripts/generate_live_deploy.sh`). Live **cycle instances reconciled** to the canonical 8 per
  SCoR via `sql/scripts/reconcile_cycle_instances.sql` — all 6 cycles now show 8. Dual-language reading is
  **CONFIG-DRIVEN via cycle instances — do NOT hardcode** ([[project_assessment_language_tracks]]).
- **Next action — #1 TOMORROW: MATH TASK INGEST.** Load the math tasks into `DimMathTask` on live (author the
  grade-level task sheets → OneLake → `EXEC usp_LoadMathTasks @SourceUri=...`; dev first, then live). Deferred
  during the 0.5.0 crunch; do this first.
- **#2 TOMORROW: IIS/ARR RESPONSE BUFFERING.** The 0.5.1 roster streaming works locally but IIS buffers the slow
  roster response on live, so the "Loading roster…" never flushes early → dead click, *only* on the slow roster
  page. Fix = disable ARR response buffering (needs IT sign-off; they're out tonight — pin the exact ARR
...
- **Queued, AWAITING user's GO before generating**: synthetic **INGEST TEST DATA** (test ingest + maintenance
  ...
  Immersion kids' literacy is FRENCH-first (not English like the English stream).
...
- **Done today** (branch `feat/math-p6-entry`, commits `4ba2d85`→`ef07b78`, **pushed**; wrap commit on top):
  - **Per-subject cycle grade bands** — Reading **P–8**, Writing **P–RG**, Math **P–6**. `/cycles` form now sets grades per subject (dynamic Grade-ranges block, defaults + editable, added `RG`); each subject-row writes its own band. `sql/scripts/set_cycle_grade_ranges_by_subject.sql` split existing dev cycles (verified). Live: run the split at cutover.
  - **Cycles list table** — dropped Subjects col (Grades now lists subject+band), stacked dates + Edit/Deactivate, `table-layout:fixed` so actions stay in-card and dividers align, vertical-centered cells.
  - **window → cycle** user-facing wording on Data Entry group-select + roster pages (internal identifiers untouched). App renamed **"Short Cycles of Response App"**; "assessment" being removed from UI/doc copy — [[feedback_avoid_assessment_term]].
  - **7 ASD-STE100 user guides** — `docs/user-guides/*.docx` (access, reading, writing, math, cohort, student, IPP). Generated as Open XML (Word COM hung in non-interactive session); generator `scratchpad/build-guides-openxml.ps1`. Sent via SendUserFile; reworded to drop "assessment".
  - **Dev troubleshooting → fixed:** teachers saw no open cycle because dev `FactEnrollment` was all 2025-2026 (expired). Root-caused with `sql/scripts/{who_sees_open_cycle,classroom_teacher_grades,staff_roster_linkage,enrollment_currency}_dev.sql`; fixed with `rollforward_enrollment_dev.sql` (+1yr). Now Aurora/Bryce/Cedar (P–6) resolve all 3 subjects, Elder Spruce (7–8) Reading+Writing, Daphne Oak (gr 9) Writing. Captured as [[project_dev_enrollment_year_rollover]].
- **Open items surfaced:** grade-8 reading benchmarks not seeded (grade-8 reading entry works but null delta — tolerated); optional live rename of "Assessment history"/"ASSESSED"; optional dev grade-promotion (students still at prior-year grades).
- **Next action:** resume the standing plan — **Math IPP + subject adaptations**, then **cohort data-summary pages** (Math cohort + Reading tweak + combined RWM). Clarify what "all 4 items" means for the Reading summary.
- **Not done (intentional):** no PR to `main` — `feat/math-p6-entry` still mid-development.
- **Blockers:** None. DEV-only; live untouched; live-PII boundary intact.

### Left Off — 2026-09-04 — 🧩 Small-group filtering ported to Reading & Writing entry (mirrors Math); fresh-laptop container setup
- **Done today** (branch `feat/math-p6-entry`, commit `93c8c18`, **PUSHED to origin**): added the Math roster's small-group filter to the **Reading and Writing** entry grids. New shared module `webapp/src/app/enter/[windowId]/[groupKey]/smallGroup.tsx` (`useSmallGroup` hook + `SmallGroupFilter` component — grade chips for multi-grade homerooms + collapsible student picker with Select all/Clear all), wired into `RosterEntry.tsx` + `WritingRosterEntry.tsx` via `roster.filter(sg.isShown)`. Filtering is **display-only** (hidden students keep staged edits and still save). Reused Math's filter CSS; Math component left untouched. `next build` green; awdev rebuilt.
- **Ops:** ran the whole session on a fresh **container-only laptop** (no node/gh) — podman setup + the Windows `-p 127.0.0.1:PORT:3000` publish gotcha now in [[reference_podman_windows_dev_container]]. `awdev` (:3001, synthetic `.env.dev`) is up on the change.
- **Next action:** hand off to desktop — `git fetch && git switch feat/math-p6-entry`, rebuild awdev there. Then resume the 2026-09-03 plan (Math IPP + subject adaptations; cohort data-summary pages).
- **Not done (intentional):** no PR to `main` — `feat/math-p6-entry` is mid-development (Math IPP flow, cohort/RWM pages, DQ, FR seed still open); merge is the user's call.
- **Blockers:** None. DEV-only work; live-PII boundary unaffected.

### Left Off — 2026-09-03 — 🧮 Math read/write path + entry page BUILT and working on dev; big UX pass; Achievement-Level relabel
- **Done today** (branch `feat/math-p6-entry`, many commits, **UNPUSHED**): the whole **Math entry loop works end-to-end on dev**.
  - SQL: `tvf_TeacherRosterMath` (student×task roster) + `usp_UpsertMathAssessment` (dated-history 0/1 upsert, full validation), both SP-granted. `FactAssessmentMath` grain → keeps change history (like reading/writing).
  - Web: `getMathRoster` + `saveMathAssessments`; group page branches Writing/**Math**/Reading; `MathRosterEntry` (grade-stacked matrix, pulled-group filter, collapsible units, Class% heat, 3-way + IPP cell cycle, ephemeral Edit-checklist, dirty-tracked Save). `next build` green; awdev rebuilt each change.
  - UX: `/enter` Math section (was "Other"); **Comprehension → Achievement Level** (Developing 50–<75, Meeting 75–<90); partial-IPP **purple ring**; compact header (homeroom·subject right-aligned on Back row); single-grade controls on the grade line; tightened top spacing (app-wide). App-wide standard: **"Select all"/"Clear all"** ([[feedback_select_clear_all_labels]]).
  - User confirmed: "It all appears to be working as intended."
- **Known gap:** nothing creates `Subject='Math'` `FactStudentIPP` rows yet (ingest + confirm flow only do Reading/Writing) — hand-insert for dev tests; real fix = tomorrow's IPP work.
- **Next action (user's stated plan for tomorrow):** (1) **Math IPP + subject adaptations** — Math-vs-Literacy IPP creation + confirm-gate UX and per-subject handling; (2) **cohort data-summary pages** — a new **Math** cohort summary, **tweak the Reading** summary to "summarize all 4 items of that assessment" (**clarify the 4 items** — reading is a single level; may mean Writing's 4 traits), then **one combined RWM summary page** across all three subjects.
- **Also open (Math, carried):** DQ checks for `FactAssessmentMath`; FR task descriptions + full P-6 seed (more grade sheets landing); optional `DimMathComprehensionBand` → `DimMathAchievementLevel` rename.
- **Blockers:** None. DEV-only; live-PII boundary unaffected (synthetic dev).

### Left Off — 2026-09-02 (evening) — 🧮 BIG PIVOT: P-6 Math pulled into 1.0; task-based mastery data layer built + dev-proven (branch `feat/math-p6-entry`)
- **Pivot**: management condensed the timeline and pulled **P-6 Math data entry** into 1.0 (was post-MVP). All prior next-steps PARKED (Short Cycles creation, homeroom picker, hardenings, guide-in-repo).
- **Model** (from the user's Primary exemplar workbook): **task-based binary mastery** — grade outcome tasks each scored **1 can-do / 0 cannot** per student; summaries by-task (proportion → colour heatmap) and by-student-per-unit (avg → 4-tier band Emerging/Developing/Meeting/In-depth). Rides the **same Short Cycles** (tasks keyed by month); bilingual task bank (one key, EN+FR). Full detail: [[project_math_assessment_model]].
- **Built + DEV-PROVEN** (branch `feat/math-p6-entry`, commit `9c48a6b`): `DimMathTask`, `FactAssessmentMath`, `DimMathComprehensionBand`, `Stg_MathTask`, `usp_LoadMathTasks`, `seed_DimMathComprehensionBand`. Loaded Primary Term-1 on dev = **35 tasks** (Unit 1 = 13, Unit 3 = 22), verified. Dev lakehouse GUID `8c5589bd-...`.
- **In progress**: seeding grades — only Primary complete; more finishing by the hour (`EXEC usp_LoadMathTasks @SourceUri='...mathtasks/<Grade>MathTasks*'` per file).
- **Next action (user's stated focus)**: (1) the **teacher data-entry UI for Math** — student×task matrix grid grouped by unit → needs `usp_UpsertMathAssessment` + `tvf_TeacherRosterMath` + summary reads, and a Math Short Cycle + Primary roster on dev to test; (2) **IPP handling for Math vs Literacy** — student may have a Math IPP and/or a Literacy IPP (`FactStudentIPP` already per-Subject; [[project_ipp_type_labelling]] already labels "Math IPP") → clarify the confirm-gate UX.
- **Two confirmations pending**: keep binary-overwrite grain (no per-attempt history)?; show `AnswerKey` on the entry grid?
- **Blockers**: None. DEV-only so far; live-PII boundary unaffected (synthetic dev).

### Left Off — 2026-09-02 (🟢 AccessLevel-NULL fixed + LIVE → /students populated; image slimmed 585→421 MB; NEW BUILD `c30095b` SWAPPED IN ON data.tcrce.ca; committed direct to `main`)
- **Last completed** (all on `main`; PRs #26 + #27 merged; several docs/memory commits direct to main — **NOT yet pushed to origin at time of this note; push as part of handoff**):
  - **AccessLevel-NULL fix** (`usp_MergeStaff`) — active-school filter moved OFF `Wrk_StaffAssignment`/`AccessByEmail` (was nulling a district analyst's AccessLevel; their role sits at SchoolID `0000`) ONTO the `FactStaffAssignment` insert only; 5b now also closes fact rows at since-closed schools. **User deployed to live + re-ingested → `/students` now populated.** ✅
  - **Post-login refresh** (`PostLoginRefresh`, entra-only) so gated nav (Cycles/Ingest) resolves without a manual reload; `/students` subtitle reworded.
  - **Dev-only impersonation bar** — run the dev app as any synthetic staff for how-to docs (cookie honoured ONLY in dev branch; inert on live). Rebuilt/verified `awdev`.
  - **Image slim-down 585→421 MB** — pruned `@next/swc`/sharp/typescript in the Dockerfile `proddeps` stage (tedious dynamic-require verified intact).
  - **PROD IMAGE SWAP on data.tcrce.ca** — `aw` container `f4432b6`→`c30095b`, verified `/api/health` 200 + running `:c30095b`; old `:f4432b6` kept for rollback. Runbook written: `docs/prod-container-swap.md`. Prod = rootless podman/`appuser`, IIS→127.0.0.1:3000, 3 podman secrets via `*_FILE`.
- **In progress**: none — deploy landed clean.
- **Next action**: (1) **USER**: browser sign-in test at `https://data.tcrce.ca` + open a data screen to confirm the full IIS→container→Fabric path on the new build. (2) **IT**: update their DR "container missing" relaunch command image ref → `localhost/assessment-webapp:c30095b`. (3) Create the first **Short Cycles** via `/cycles` so `/enter` is populated for teachers.
- **Also pending** (unchanged): homeroom-chips searchable picker ([[project_homeroom_chips_unwieldy]]); image version scheme ([[project_image_versioning_scheme]]); the how-to guide (Claude artifact, still in scratchpad — decide if it goes in the repo); optional hardenings (CSP, SP `ReadData` least-privilege).
- **Blockers**: None. (Live-PII boundary in effect — schema/synthetic only, user runs live checks.)

### Left Off — 2026-08-27 (🟢 PRODUCTION CUTOVER COMPLETE + first real ingest CLEAN; Short Cycles pivot + capability model live; NEXT: student data shows in warehouse but NOT in the app)
- **Last completed** (huge session, all committed on `phase-3b-webapp-b6`, pushed): (1) Writing web UI finished (detail toggle) + subject persistence. (2) **Auth hardening** — login/data Entra app split (`AUTH_ENTRA_*` vs `ENTRA_*`, no fallback), file-mounted secrets (`load-secrets.cjs`), `.dockerignore` `.env*`, AUTH_URL/redirect docs. (3) **App deployed to production by IT** at on-prem **data.tcrce.ca** (Canadian, internal-only) — but run LOCALLY via `awlive` on `webapp/.env` for now (web-app image redeploy deferred). (4) **Short Cycles of Response pivot** — manual region-wide multi-subject cycles (`CycleGroupID`), per-student reading scale, explicit BenchmarkMonth, `/cycles` admin screen, terminology/brand rename. (5) **StaffAppAccess capability model** — `IsSysAdmin`/`CanManageCycles`/`CanRunIngest`; `usp_TriggerIngestCycle` gates on it (sysAdmin bootstrap on empty roster). (6) **PRODUCTION DATA CUTOVER** — `reset_for_production.sql` then debugged the first real PS ingest to a **clean DQ-passing run** (base-name loader globs, trailing-empty-line junk-row guards, closed-school/`0000` access filtering, staff-assignment dedup, section-teacher resolution, 2 alt schools 1254/1255). Live now holds REAL student PII → [[feedback_live_pii_boundary]] fully in effect. See [[project_assessment_platform]] for the full cutover record + ingest gotchas.
- **In progress**: none — ingest is clean, real rosters on live.
- **Next action** (TOMORROW, START HERE): **student data appears in the warehouse tables but NOT in the app's Student Data (`/students`) screen** for the signed-in user. Almost certainly an `@UPN` / `AccessLevel` scoping issue — check that jeffrey.raine's ingested `DimStaff` row resolved to `AccessLevel='RegionalAnalyst'` (Group 40) and that the UPN the app passes (via `awlive` sign-in) matches `DimStaff.Email` (case/format). `tvf_StudentCohort` RegionalAnalyst branch requires `EXISTS (DimStaff WHERE LOWER(Email)=LOWER(@UPN) AND IsCurrent=1 AND AccessLevel='RegionalAnalyst')`. Debug on schema/synthetic — NO live row-level PII queries; have the user run any live checks.
- **Then**: create the first Short Cycles (`/cycles`) so `/enter` is populated for teachers; web-app image → data.tcrce.ca; optional hardenings (CSP, SP `ReadData`).
- **Blockers**: None.

### Left Off — 2026-08-26 (back from summer break; Writing feature COMPLETE end-to-end — SQL live + web UI built/committed; subject now persists across cohort→detail nav; dev container running; only the live image refresh remains)
- **This session (2026-08-26, back from the summer break)**: session-start + brought memory/plan current to the git log (the Writing work below predated any wrap); produced a C-suite build timeline (neutral/passive voice); spun the **dev container** back up (`podman machine start`; `awdev` :3001 on synthetic `_Dev` data — SP secret survived the 2-month gap, `/enter` reads live); **fixed subject persistence** — clicking a student from the Writing cohort now opens the detail on the Writing tab and Back returns to the Writing cohort (`3543c7f`, `CohortView.tsx` + `StudentDetailView.tsx`; image rebuilt → `e494e38`, tagged `:token`, verified on dev); built two **reading-benchmark reference spreadsheets** (`docs/reference/reading_benchmarks_{immersion,english}.csv` — P–6 × Sept–Jun, min/max cells, parsed from the seed matrices).
- **Last completed** (all on branch `phase-3b-webapp-b6`, committed; reconstructed from git log after the summer break — the 06-25 note below predates this work): the **entire Writing web UI** now mirrors Reading and is subject-aware —
  - **Entry (W4b/c, `b37b8f3`)**: `/enter` roster grid branches on `AssessmentType='Writing'` to a 4-input (1-4) grid per student — `WritingRosterEntry.tsx` + `saveWritingAssessments` action + `getTeacherRosterWriting`; live avg + band, inline Writing-IPP confirm.
  - **Cohort (W4d, `e235fc8`)**: `/students` Reading|Writing `?subject` toggle (avg→band donut/bar/table).
  - **Detail (W4d, `73120b6`)**: student-detail Reading|Writing toggle — 4-trait history table + "writing average over time" trend; `getStudentHistoryWriting` + `loadStudentHistoryWriting`; `getStudentNavList(upn, subject)`; `StudentNavItem.ippStatusReading` → generic `ippStatus`.
  - **Window-select grouping (`fa23197`,`59278ac`)**: `/enter` windows split under **Reading / Writing** sub-headings; the per-section "N upcoming windows" note removed.
  - **Entered-count fix (`d13d70b`)**: `tvf_UserAssessmentWindows` + `tvf_TeacherGroups` count `EnteredStudentCount` by window **type** (Reading→`FactAssessmentReading`, Writing→`FactAssessmentWriting`) — writing windows used to always show 0 entered. (These two do NOT self-grant → re-run `grant_webapp_sp.sql` after redeploy.)
  - **Pool self-heal (`40bf788`)**: `webapp/src/lib/db.ts` `runOnPool()` resets + retries once on stale SP-token / dropped-connection errors (fixes the recurring dev "authentication failed" after the ~1h Fabric token expiry). Retry-safe for writes (procs idempotent on Student×Window×Date).
  - **Writing SQL → LIVE**: user deployed the whole warehouse side ("everything on the warehouse side has been deployed") — `usp_UpsertWritingAssessment`, the three `tvf_*Writing`, DQ writing checks, `usp_GenerateMonthlyWindows` (writing scopes) + `EXEC '2026-2027'`, `grant_webapp_sp.sql`. Writing windows confirmed present on live.
- **In progress**: none — the Writing feature is functionally complete (entry + cohort + detail all verified on dev; SQL live).
- **Next action**: **Refresh the live container (`awlive`) onto the current image (`e494e38` `:token`)** so the writing UI + subject-persistence fix reach live — **image-only, no SQL** (writing TVFs already live). `awdev` (:3001) is already running on this image; `awlive` (:3000) is stopped.
- **Then (unchanged, not blocking)**: web-app image → a Canadian host (still local-only via Podman `awlive`/`awdev`); optional hardenings — drop broad SP `ReadData` on live (ownership chaining verified on dev), CSP in `next.config`, postcss transitive vulns.
- **Blockers**: None. (Entra admin consent for the OBO/user-token path still pending — AADSTS65001 — off the critical path; SP+`@UPN` works at $0. Probe parked on `entra-obo-probe`.)

### Left Off — 2026-06-25 (SCD same-day family + monthly-window model LIVE; Writing feature SQL done/dev-proven; Writing web UI is the only piece left)
- **Last completed**: A big session. **To LIVE:** the SCD same-day fix + same-day **revival** (drop-then-re-add) + staff HomeSchoolID `'0000'` fix (all 4 merge procs); the **ongoing-assessment monthly-window model** ([[project_ongoing_assessment_model]]) — `usp_GenerateMonthlyWindows`, reworked `usp_UpsertReadingAssessment` (grain Student×Window×Date, multiple dated entries, late entry), `/enter` accordion UI — with 2026-2027 windows generated. Web chrome: home page reworded to the Power App landing, "Assessment Data" → **"Data Platform"**, "Enter Assessments" → **"Data Entry"**, Ingest nav/card **role-gated** to analysts. **On DEV only:** the entire **Writing** feature SQL — `usp_UpsertWritingAssessment` (4-trait 1-4, avg→band by cut scores 3.50/2.75/1.75), `tvf_StudentCohortWriting` / `tvf_StudentAssessmentHistoryWriting` / `tvf_TeacherRosterWriting`, DQ writing checks, generator writing scopes — all dev-proven; plus a **fix to `tvf_TeacherRoster`** (reading) for the multiple-entry duplicate-row bug.
- **In progress**: Writing feature — **SQL complete + dev-proven; the web UI (W4b–d) is NOT started** (deferred by user: "leave the UI for later").
- **Next action**: Build the Writing **web UI** — the four 1-4 inputs per student on the roster grid when the window is `AssessmentType='Writing'` (new component + `saveWritingAssessments` action + `getTeacherRosterWriting`), and the **Reading|Writing toggle** on the cohort + student-detail screens. THEN deploy the Writing SQL + the `tvf_TeacherRoster` dup-fix to **live**, and the web-app image to a Canadian host.
- **Blockers**: None. (Entra admin consent for the OBO/user-token path is still pending — AADSTS65001 — but it's not on the critical path; SP+`@UPN` works at $0. Probe parked on branch `entra-obo-probe`.)

### Left Off — 2026-06-23 (Phase 3b: B6 screens + in-app ingest + dev env + security pass DONE; SCD same-day fix in source → DEPLOY FIRST NEXT SESSION)
- **Last completed**: **Phase 3b B6 + supporting work, all proven on dev.** Web-app screens built and bound to live Fabric on the SP + `@UPN` pattern: roster grid with staged IPP confirm ("Yes (Literacy IPP)", commits on Save; IPP shown in Expected & Δ), dedicated IPP manager, cohort (`/students`), student detail with client-side prev/next neighbour prefetch, and **in-app ingest** (upload PS export → OneLake ADLS REST → `usp_TriggerIngestCycle`). **Security pass** (10-item code audit + fixes: fail-closed `AUTH_MODE`, `UserError`/`toUserMessage`, security headers, generic health/status, deleted `/api/dbcheck`+`mock.ts`). **Dev environment** stood up (synthetic Dev warehouse+lakehouse, `_Dev` items, same workspace; `.env` swap) and validated app-driven ingest end-to-end → **DQ PASS**. Diagnosed the dev same-day reversed-window DQ failure → it's a real **SCD merge-proc bug**, now fixed in source (all 4 merge procs: same-day change updates current row IN PLACE; see [[project_scd_same_day_reversion_fix]]).
- **In progress**: **SCD same-day-reversion fix is in SOURCE only — not deployed anywhere.** Procs edited + self-dropping: `usp_MergeStudent.sql`, `usp_MergeStaff.sql`, `usp_MergeSection.sql`, `usp_MergeSectionTeachers.sql`.
- **Next action (DEPLOY FIRST, in order)**:
  1. Deploy the **4 fixed merge procs to DEV**, then test: re-upload the `_SCDTest` students file (same-day homeroom change) + run ingest → expect **DQ PASS** + one current `DimStudent` row updated in place (`StudentNumber 9100000001`, Homeroom `P-Updated`, `EffectiveEndDate` NULL, no reversed window). Restore the main students file afterwards (back to P-Sample = 2).
  2. Deploy the **4 fixed merge procs to LIVE** (same bug exists there).
  3. Deploy **`sql/security/grant_webapp_sp.sql`** (least-privilege) to dev → verify reads still work via ownership chaining with broad ReadData removed → then live.
  4. Deploy the new `@UPN` iTVFs + `@CallerUPN` procs (cohort/detail/IPP/ingest-trigger) to **live**, and rebuild + deploy the web-app image + `.env.live` to the Canadian host.
- **Blockers**: None for the merge-proc deploy. (Deferred, non-blocking: add `:3001` to the SP's Entra redirect URIs if dev should use real Entra; optional #3 proc-layer scope hardening; #10 postcss transitive vulns.)

### Left Off — 2026-06-19 (Phase 3b: B3 read + B4 write + B5 first real screen ALL PROVEN; OBO blocked on admin consent only)
- **Last completed**: **B3/B4/B5 on the web app, end-to-end against live Fabric.** B3 read (`/api/dbcheck` → `connected_as:"StudentDataAssessment"`); B4 write (`usp_InsertSubmissionAudit` via the SP — row landed + read back, confirms EXECUTE grant + ownership chaining); B5 first real screen (`/enter` window+group select bound to live data via `src/lib/data.ts`, verified as `classroom.teacher1@tcrce.ca` → 2 real windows + "Homeroom 6A", `@UPN`-scoped). User **deployed `sql/security/bridge_views.sql`**.
- **Connection root cause + fix** (full detail in memory [[project_webapp_fabric_connection]]): the `ESOCKET "socket hang up"` was **`tedious` 18 failing Fabric's TDS login — `tedious` 19 fixes it** (PR #1668), so **`mssql@^12`** not `^11`. Also: mint the token with `@azure/identity` (`azure-active-directory-access-token`), not tedious's SP-secret flow; and `serverExternalPackages` + ship `node_modules` (Next can't trace tedious's dynamic require). ODBC/`msnodesqlv8` explored first — segfaults on Linux AAD; abandoned. Milestone tag `webapp-v0.1-tedious-baseline` (commit `55f7627`) preserves the pre-swap state.
- **OBO / user-token path — tested, blocked on admin consent ONLY**: requesting the Fabric SQL scope at sign-in returns "Need admin approval" / `AADSTS65001`. IT had already **added** `user_impersonation` (delegated, Azure SQL Database), so the lone remaining step is **"Grant admin consent for TCRCE"**. NOT pursued — the SP+`@UPN` model already scopes at $0, and OBO raises a separate per-teacher-Fabric-access licensing question. Probe fully reverted (auth.ts on default scope; route deleted).
- **Next action (Monday 2026-06-22)**: **B6** — port the remaining screens to live Fabric on the SP+`@UPN` pattern (same as B5): roster grid (`/enter/[windowId]/[groupKey]`) reading per-student rows from `vw_BridgeTeacherRosterAll` + write via `execProc`(`usp_UpsertReadingAssessment`); student detail (`/students/[studentKey]` off `vw_BridgeStudentCohortAll` / `vw_BridgeAssessmentHistoryAll`); IPP (`usp_UpsertStudentIPP`). Then remove the temp `/api/dbcheck` probe before any deploy.
- **Notes**: `webapp/.env` holds the working SP creds + Fabric server — never commit it (SP display name `StudentDataAssessment`). USER (not blocking B6): send the bundled web-app IT request for the production app reg; open the PR.

### Left Off — 2026-06-18 (PHASE 3 FORK → 3b web app: container + Entra wiring + full nav/layout scaffold + TCRCE branding; app reg confirmed IT-gated)
- **Last completed**: Phase 3b built out in [`webapp/`](../webapp/), all verified under Podman — (1) Next.js 15 + TS container skeleton (server-only Fabric `queryAsUser` w/ `@UPN`, surrogate-key string guards, `dev`/`entra` auth modes, committed lockfile); (2) **B2 Entra login wiring** (Auth.js v5 + Entra ID provider, session UPN, header access-control widget) — compiles + runs, **OAuth round-trip UNTESTED**; (3) **B-scaffold** — app shell + nav + a route for all 8 screens with placeholder data (all routes 200); (4) **TCRCE logo + favicon package** wired. ~8 commits, branch pushed.
- **Hard blocker found**: app registration is **IT-only in this tenant** — user hit 401 on the App registrations blade (memory [[project_entra_appreg_it_gated]]). So B2's round-trip AND B3/B4's Fabric token both wait on IT. Bundled one-pass request ready: [`docs/it-request-entra-webapp-dev.md`](it-request-entra-webapp-dev.md) (app reg + sign-in + delegated Azure SQL `user_impersonation` + admin consent). Hosting brief for `data.tcrce.ca` ready: [`docs/brief-server-hosting-requirements.md`](brief-server-hosting-requirements.md).
- **Next action**: USER — (1) **send the bundled web-app IT request** (critical path; HOLD the 3a bridge request); (2) open the PR (gh unauthenticated): https://github.com/MrJRaine/Assessment-Data/pull/new/phase-3b-webapp; (3) optionally prove B2 mechanics in a free/dev Entra tenant. CLAUDE — wire the `queryAsUser` data-access layer + typed views on spec (so B3/B5 are code-complete pending the live token), or build one screen's real controls.
- **Calendar**: both teachers AND the user are 10-month (off July–August); build happens only in-session. June = de-risk 3b + fire the IT request into the summer queue; substantive build resumes late Aug/Sept; pilot targets a fall assessment window (date TBD by user).
- **Blockers**: Entra app registration (IT-gated) — gates live auth + Fabric read/write. No SQL / warehouse changes this session.
