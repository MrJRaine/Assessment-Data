...
### Left Off — 2026-08-26 (Writing feature COMPLETE end-to-end — SQL live + web UI built/committed; only the live web-app image refresh remains)
...
- **Next action**: **Refresh the live container (`awlive`) onto the current image** so the detail toggle (+ the rest of the UI since the last live image) reaches live — **image-only, no SQL** (writing TVFs already live). After the ~2-month break the Podman containers are stopped; spin one up before testing.
