...
- **Next action — TOP PRIORITY tomorrow**: **IIS/ARR RESPONSE BUFFERING.** The 0.5.1 roster streaming works
  locally but IIS buffers the slow roster response on live, so the "Loading roster…" never flushes early →
  dead click, *only* on the slow roster page. Fix = disable ARR response buffering (needs IT sign-off; they're
  out tonight — pin the exact ARR "response buffer threshold → 0" setting). Buffering-immune alternative: re-add
  the `useLinkStatus` client overlay on the cards as **0.5.2** (user deferred building it tonight).
