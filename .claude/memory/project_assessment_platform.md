...
- **Web-app fork (Phase 3b)**: `webapp/` — Next.js 15 + TS, containerized (Podman/Alpine). **B2–B6 all PROVEN against live Fabric.** Connection: `@azure/identity` ClientSecretCredential token + `mssql@12` (tedious 19, NOT 11/tedious 18) as the SP; reads `@UPN`-scoped inline TVFs / bridge views; writes via wrapper procs (ownership chaining). **Screens built (2026-06-22→23):** roster grid with *staged* IPP confirm (Yes = "Literacy IPP"/"Math IPP"; commits on Save, no per-click freeze) + IPP shown in Expected & Δ; dedicated IPP manager; cohort (`/students`); student detail with client-side prev/next neighbour prefetch; **in-app ingest** (upload PS export → OneLake ADLS REST → `usp_TriggerIngestCycle`). **Security pass done (2026-06-23):** fail-closed `AUTH_MODE` (`ALLOW_DEV_AUTH` opt-in), `UserError`/`toUserMessage` PII-safe errors, baseline security headers, generic health/status in prod, deleted `/api/dbcheck` + `mock.ts`. **Dev env (2026-06-23):** synthetic Dev warehouse+lakehouse (`_Dev` items, same workspace) is Claude's sandbox; `.env` swap for live — [[project_dev_live_environment_split]], [[feedback_live_pii_boundary]]. Not logged in → redirect to Entra sign-in. Branch `phase-3b-webapp`. See [[project_webapp_fabric_connection]].
- **PENDING DEPLOY — first thing next session (dev → test → live):** (1) **SCD same-day re-version fix** — all 4 merge procs (`usp_MergeStudent/Staff/Section/SectionTeachers`) hardened so a same-day attribute change UPDATES the current row IN PLACE instead of close+insert (close+insert on the same day the row was created produced the reversed-window `EffectiveEndDate < EffectiveStartDate` + self-overlap DQ failures observed on dev 2026-06-23; the truncate-all reset cleared the data but the proc bug remains in live). Source fixed + self-dropping; NOT yet deployed anywhere — see [[project_scd_same_day_reversion_fix]]. (2) **`grant_webapp_sp.sql`** least-privilege (explicit grants + DENY bridge views + remove broad ReadData — verify ownership chaining on dev first). (3) the new `@UPN` iTVFs + `@CallerUPN` procs (cohort/detail/IPP/ingest-trigger) to **live**. (4) rebuild + deploy the web-app image + `.env.live` to the Canadian host.
