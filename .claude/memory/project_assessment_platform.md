...
- **🔴 PRODUCTION DATA CUTOVER COMPLETE (2026-08-27):** live warehouse RESET (`reset_for_production.sql`) then the **first real PowerSchool ingest ran successfully** (5 sqlReports → students/staff/sections/enrollments/section-teachers). Live now holds **REAL student PII** — [[feedback_live_pii_boundary]] is FULLY IN EFFECT: no row-level PII queries against live, no headless nav of live data screens; operate on schema + synthetic (`_Dev`) only. The Short Cycles pivot + StaffAppAccess capability model (sysAdmin bootstrap) are on live. The app is run for now from a LOCAL `awlive` container (image current) against the live warehouse, using `webapp/.env` (complete: live DB, real login app, inline secrets, localhost:3000) — NOT `.env.live` (server template, secrets stripped). The web-app image to `data.tcrce.ca` is deferred (user can't redeploy that container right now).
