...
- **Licensing pivot build (ACTIVE):** SharePoint-list rework of the teacher entry path per [[project_licensing_pivot_2026_06]] — bridge variant choice (Automate vs Fabric-side), list permission/PIIDPA design, validation error-sync-back, count tripwires. June pilot timing depends on this (~3-5 wks).
- **User's Power Apps premium trial expiry** — being verified; when it lapses, dev access to the SQL-bound app stops. Mitigate via PAYG link or one maker license.
