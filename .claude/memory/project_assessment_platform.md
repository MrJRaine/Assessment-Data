...
- **Writing WEB UI — BUILT + COMMITTED (branch `phase-3b-webapp-b6`), image NOT yet on live:** entry grid (`WritingRosterEntry` + `saveWritingAssessments`), `/students` cohort `?subject` toggle, student-detail toggle (4-trait history + avg-over-time trend), `/enter` windows grouped under Reading/Writing sub-headings, and `db.ts` `runOnPool()` pool self-heal (retry-once on stale SP token — fixes the recurring dev "authentication failed"). All dev-verified.
- **PENDING DEPLOY:** (1) **Web-app image → live/Canadian host** — the committed web UI (Writing entry+cohort+detail, window grouping, pool self-heal, subject-persistent cohort→detail nav) reaches live only when `awlive` is refreshed onto the current image (`:token` = `e494e38` as of 2026-08-26); **image-only, no new SQL** (writing TVFs already live). The 3b app still runs local-only (`awlive`/`awdev` via Podman; `awdev` :3001 back up 2026-08-26, `awlive` :3000 stopped). (2) Optional: SP least-privilege — drop broad `ReadData` on live (ownership chaining verified on dev 2026-06-25, rows still return); CSP in `next.config`; postcss transitive vulns. See [[project_scd_same_day_reversion_fix]].
