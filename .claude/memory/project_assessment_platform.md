...
- **Web-app fork (Phase 3b, 2026-06-18→19)**: `webapp/` — Next.js 15 + TS, containerized (Podman/Alpine). **B2 login, B3 read, B4 write, B5 first screen all PROVEN end-to-end against live Fabric.** Connection: `@azure/identity` ClientSecretCredential token + `mssql@12` (tedious 19, NOT 11/tedious 18) as the SP; reads bridge views filtered by `@UPN`; writes via wrapper procs (EXECUTE grant in `sql/security/grant_webapp_sp.sql`, ownership chaining). NOT done: B6 (roster/student-detail/IPP screens), deploy to a Canadian host, Teams embed. Temp `/api/dbcheck` to remove pre-deploy. Branch `phase-3b-webapp` (PR open). See [[project_webapp_fabric_connection]].
