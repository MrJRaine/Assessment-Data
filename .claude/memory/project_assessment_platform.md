...
- **PENDING DEPLOY:** (1) **Web-app image → live/Canadian host** — the committed web UI (Writing entry+cohort+detail, window grouping, pool self-heal) reaches live only when `awlive` is refreshed onto the current image; **image-only, no new SQL** (writing TVFs already live). The 3b app still runs local-only (`awlive`/`awdev` via Podman; stopped after the summer break — spin up before testing). (2) Optional: SP least-privilege — drop broad `ReadData` on live (ownership chaining verified on dev 2026-06-25, rows still return); CSP in `next.config`; postcss transitive vulns. See [[project_scd_same_day_reversion_fix]].
