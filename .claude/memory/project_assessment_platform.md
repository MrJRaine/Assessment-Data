...
- **PENDING DEPLOY:** (1) **Writing SQL → live** — `usp_UpsertWritingAssessment`, `tvf_StudentCohortWriting`, `tvf_StudentAssessmentHistoryWriting`, `tvf_TeacherRosterWriting`, the DQ proc (3 writing checks), `usp_GenerateMonthlyWindows` (now has writing scopes); then `grant_webapp_sp.sql` + `EXEC usp_GenerateMonthlyWindows '2026-2027'` (adds the writing windows). (2) ~~Reading-roster dup-row fix~~ — **DONE: deployed to live 2026-06-25** (`tvf_TeacherRoster` latest-per-window). (3) **Web-app image → Canadian host** (the 3b app still runs local-only; `awlive`/`awdev` containers via Podman). (4) Optional: SP least-privilege — drop broad `ReadData` on live (ownership chaining verified on dev 2026-06-25, rows still return). See [[project_scd_same_day_reversion_fix]].
