...
**Reinforced 2026-09-04 ("stop being a quitter").** A `git push` failed with *"Cannot prompt because user interactivity has been disabled"* and I declared it blocked and handed it back. The fix was one retry with the helper's interactivity re-enabled: `$env:GIT_TERMINAL_PROMPT="1"; $env:GCM_INTERACTIVE="true"; git push …` — it went straight through. The harness suppresses interactive prompts by default; treat that as a default to override, not a wall. Rule: when an action is "blocked" only by a non-interactive/auth guard, attempt the obvious override once (with a bounded timeout so it can't hang unwatched) before handing the task back.
