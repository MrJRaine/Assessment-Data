...
modified: 2026-09-09T18:45:47.281Z
---
...
## Session 2026-09-02 — AccessLevel-NULL fix (live), post-login refresh, dev impersonation bar, image slim-down, PROD image swap on data.tcrce.ca

**Resolved the "students in warehouse but not in app" issue.** Root cause = `DimStaff.AccessLevel = NULL` for the signed-in RegionalAnalyst, and it was MY regression from the cutover session: the active-school `WHERE EXISTS (DimSchool ActiveFlag=1)` filter I added to the `Wrk_StaffAssignment` build in `usp_MergeStaff` ALSO feeds the `AccessByEmail` CTE that computes AccessLevel. A district analyst's role assignment sits at SchoolID `0000` (not an "active school"), so the filter stripped it from the AccessLevel compute → AccessLevel NULL → `tvf_StudentCohort`'s RegionalAnalyst branch returned nothing. **Fix:** removed the active-school filter from the `Wrk_StaffAssignment` build (kept blank-email guard + `(Email,School,Role)` dedup), added it to the `FactStaffAssignment` insert (5c) ONLY, and made 5b also close fact rows whose school has since closed (so the fact still self-heals on closure, which the Wrk-level filter used to provide implicitly). HomeSchoolID NULL for a district analyst is EXPECTED/correct. **User deployed `usp_MergeStaff` to live + re-ingested → `/students` populated.** (Confirmed working; the full real dataset is what surfaced the homeroom-chips UX note.)

**Web-app changes (all merged to main via PR #26):** (1) **one-time post-login refresh** — the root layout only re-renders on a full document load, so StaffAppAccess-gated nav (Cycles/Ingest) lagged a just-established session (observed: sysAdmin granted mid-session left the nav stale until manual refresh). `PostLoginRefresh` (entra-mode-only client component) forces one `router.refresh()` on the first authenticated render per browser session, guarded by sessionStorage, cleared on sign-out. (2) `/students` subtitle reworded to "drill down to the student level". (3) **DEV-ONLY impersonation bar** — run the dev app as any synthetic teacher/admin/analyst without editing DEV_FAKE_UPN + restarting, for making how-to docs. `getCurrentUpn()` honours a `dev_impersonate_upn` cookie ONLY inside the dev-mode branch; `setImpersonation`/`clearImpersonation` server actions hard-gate on dev → inert on entra/live. Picker lists synthetic teachers-with-rosters + privileged staff (`getImpersonationTargets`) + free-text UPN + reset. `AuthArea` shows the effective (impersonated) UPN; bar rendered ABOVE the header for clean screenshots. Rebuilt+verified `awdev`.

**Image slim-down (PR #27, merged).** Investigated the 585 MB image: `@next/swc-linux-x64-musl` was **136 MB** of build-only compiler riding along because we ship the full prod node_modules (for mssql/tedious's dynamic require). Pruned `@next/swc*` + sharp/@img (no next/image; plain `<img>`) + typescript/@types in the `proddeps` stage (an `rm` in the runner can't reclaim an earlier COPY layer — OCI layers are additive). **585 → 421 MB (−28%).** Verified: build passes, `awdev`/`awlive` serve 200 incl. `/students`, tedious dynamic require intact.

**PROD DEPLOY — swapped `data.tcrce.ca` to the new build.** Built `assessment-webapp:token` from main (`c30095b`), `podman save` → `assessment-webapp-c30095b.tar` (402 MB, on the DEV box at `C:\Git-Repos\`). IT placed both the old (`-f4432b6`) and new (`-c30095b`) tars in prod `C:\temp`. Learned the real prod setup from IT's server-recovery doc: container `aw`, rootless podman as `appuser` (uid 1001, `XDG_RUNTIME_DIR=/run/user/1001`), env `/mnt/c/temp/.env.live`, 3 podman secrets via `*_FILE`, `127.0.0.1:3000` behind IIS (`W3SVC`, `preserveHostHeader=True`), health `/api/health`, `--restart=unless-stopped`. Verified in-repo that `/api/health` exists + `load-secrets.cjs` maps the exact 3 `*_FILE` vars → drop-in. Swap sequence executed with the user: preflight (3 secrets present, current image f3d0c737dafc) → `podman load` new tar (`:token`) → `podman tag …:token …:c30095b` → stop/rm/run `aw` on `:c30095b` → verify `podman ps` shows `:c30095b Up` + `/api/health` = 200 `{"status":"ok"}`. Old image kept as `:f4432b6` for rollback. **Wrote `docs/prod-container-swap.md`** (committed to main) — the generalized runbook. TODO for IT: point their DR "container missing" relaunch command at `localhost/assessment-webapp:c30095b` (was pinned to the old image id).

**Repo hygiene:** PRs #26 + #27 merged to main; deleted merged branches (phase-3b-webapp-b6, phase-3-power-apps, phase-3b-webapp, session-2026-04-29/30, step-14-data-quality-checks) local+remote; deleted the stale local-only `entra-obo-probe` (single PARKED OBO-auth spike from 2026-06-24, 74 behind main — user OK'd; SHA `3c07151` recoverable from reflog ~30d). Repo now `main` only.

**New memories:** [[project_homeroom_chips_unwieldy]] (Homeroom smart-chips unwieldy for analysts at full scale — swap for a searchable/school-scoped picker before rollout; low priority) and [[project_image_versioning_scheme]] (user asked to revisit commit-SHA tar names + mutable `:token` tag → semver tags; raise at next planning/wrap). A **user how-to guide** (sign-in + every feature) was published as a Claude artifact — still in scratchpad, NOT committed to the repo.

**Left off:** New build (`c30095b`) LIVE on `data.tcrce.ca`, `/api/health` 200. Remaining: (a) USER to do the browser sign-in + data-screen check on data.tcrce.ca to confirm the full IIS→container→Fabric path on the new build; (b) IT to update their DR relaunch command to `:c30095b`; (c) optionally SHA-tag convention going forward (see image-versioning memory); (d) still-open pre-cutover items: create the first Short Cycles via `/cycles`, homeroom-chips picker, optional hardenings (CSP, SP ReadData least-privilege), and deciding whether the how-to guide lives in the repo.

## Session 2026-09-02 (evening) — BIG PIVOT: P-6 Math pulled into 1.0; task-based mastery model designed; data layer built + dev-proven

**The pivot.** Management condensed the timeline and pulled **P-6 Math data entry** into the 1.0 rollout (was post-MVP, scoring TBD). Parked all the prior next-steps (Short Cycles creation, homeroom picker, hardenings, guide-in-repo) to focus on Math.

**Discovering the model.** The user supplied an exemplar curriculum workbook (`C:\Git-Repos\TCRCE Math Grade Primary 26-27 Assessment ... .xlsx`). Parsed it with a PowerShell xlsx-unzip script (scratchpad `parse-xlsx.ps1` → `xlsx-dump.txt`) to read values AND formulas. Model = **task-based binary mastery**: grade-level outcome **tasks**, each scored **1 can-do / 0 cannot** per student; two summaries — **by-task** proportion (colour heatmap >80/65-80/50-64/<50) and **by-student-per-unit** average → **4-tier band** (Emerging `<0.5` / Developing `≤0.75` / Meeting `<0.9` / In-depth `≥0.9`, "Incomplete" if `<80%` of unit tasks scored). KEY CLARIFICATIONS from the user: the exemplar's Term-1 format is authoritative (Term 2/3 were half-revised — 7-level scale ignored, we're on the **4-level**); Math rides the **same Short Cycles** (the term split was formatting only), tasks keyed by **month**; tasks grouped in **units** (page sub-headings); `QuestionNumber` ('2a'/'2b') separate from display order; `GradeLevelOutcome` (`N02.01`) its own column; one bilingual task table (EN+FR columns, one key) — program = display language only.

**Built (branch `feat/math-p6-entry`, commit `9c48a6b`, DEV only):** `DimMathTask` (bilingual bank, NK `(GradeCode, AssessmentMonth, UnitName, QuestionNumber)`), `FactAssessmentMath` (grain Student×Window×Task → `Result BIT`; binary overwrite, no history — flagged to confirm), `DimMathComprehensionBand` (4 tiers, code→label+colour; thresholds in the view), `Stg_MathTask` + `usp_LoadMathTasks` (seed from a grade CSV that mirrors the table: `COPY INTO` via parameterized `sp_executesql` path + Type-1 upsert; retire scoped to the batch's Grade+Month), `seed_DimMathComprehensionBand`. Seed template authored at `C:\Git-Repos\DimMathTask-seed-template.csv` (user then edited it; AssessmentMonth=9). **Dev deploy proven:** loaded Primary Term-1 → **35 tasks (Unit 1 = 13, Unit 3 = 22)**, verified. Dev lakehouse GUID `8c5589bd-d04e-4e94-bb2c-482db645afab` (from `.env.dev`; workspace `a1b49041-...` same dev/live). Hit the familiar 0-row silent COPY INTO once (filename didn't match the `PrimaryMathTasks*` wildcard) — fixed by naming.

**Full model detail → new memory [[project_math_assessment_model]].**

**Left off:** Math data layer LIVE on DEV, Primary tasks seeded. Only the Primary sheet is complete; more grades finishing by the hour (same one-line `EXEC usp_LoadMathTasks` per file). **NEXT SESSION (user's stated focus):** (1) the **teacher data-entry UI for Math** — the student×task matrix grid grouped by unit; needs `usp_UpsertMathAssessment` + `tvf_TeacherRosterMath` + the by-task / by-student-per-unit summary reads, and a Math Short Cycle + Primary roster stood up on dev to test; (2) **IPP handling now that Math is a distinct type** — a student may have a **Math IPP and/or a Literacy IPP**; `FactStudentIPP` is already per-`Subject` and [[project_ipp_type_labelling]] already labels "Math IPP", so this is clarifying the confirm-gate UX across both. Two confirmations still pending: keep the **binary-overwrite** grain (no per-attempt history), and **show `AnswerKey`** on the entry grid as a marking reference.

## Session 2026-09-03 — Math read/write path + entry page BUILT and working on dev; big UX iteration; Achievement-Level relabel

**History-grain decision.** Reversed the parked "binary overwrite" question: `FactAssessmentMath` now **keeps change history like reading/writing** (grain Student×Window×Task×**Date**, latest-by-date wins). Columns already supported it — doc/header change only.

**SQL read/write path** (mirrors the reading TVF/proc). `tvf_TeacherRosterMath` — same 3 role branches as `tvf_TeacherRoster`, but each student joins to THEIR grade's `DimMathTask` for the cycle's month → one row per (student × task), with latest result + Math-IPP + program-language task text. `usp_UpsertMathAssessment` — dated-history upsert of a 0/1 (or clear), full validation ladder (51010-51032 + 51018 task grade/month + 51019 value), `MIN(today,EndDate)` date cap; both SP-granted. Committed `b6a1b53`.

**Web entry page** (`89d9fdb` + fixes). `data.ts` `getMathRoster`, `actions.ts` `saveMathAssessments` (server-resolved UPN + per-student scope gate), the group page branches Writing/**Math**/Reading, and `MathRosterEntry.tsx` — the approved mockup ported to React: grades stacked (filter + whole-grade collapse), pulled-group student picker, collapsible units with mirrored bands, Class% heat, 3-way + IPP cell cycle, ephemeral Edit-checklist (Select all/Clear all everywhere — new app-wide standard [[feedback_select_clear_all_labels]]), dirty-tracked Save. `next build` green; awdev rebuilt.

**Verified working end-to-end on dev** (user deployed the tvf+proc, created a Sept Math cycle, has a Primary roster): seed → cycle → matrix → save → reload. "It all appears to be working as intended."

**UX iterations this session (all committed + rebuilt):** `/enter` gives Math its own section (was "Other"); relabel **Comprehension → Achievement Level** + shifted boundaries (Developing 50–<75, Meeting 75–<90); IPP handling — student with a mix of IPP-omitted + assessed tasks gets a **purple ring** (4px) on the calculated level (all-IPP = plain IPP flag, no-IPP = normal); dropped the "Roster entry" header (homeroom·subject right-aligned on the Back-to-groups row); single-grade homerooms put tasks-shown + Edit + Save on the grade heading line; partial-IPP legend = neutral ringed swatch "Some tasks omitted"; tightened page-top spacing (container/PageHeader app-wide); removed a stray Class%-legend margin.

**IPP gap found:** nothing creates `Subject='Math'` `FactStudentIPP` rows yet (ingest auto-IPP + the confirm flow only do Reading/Writing). Gave a hand-insert for dev testing; the real fix is part of tomorrow's IPP work.

**Left off:** Math entry path DONE + working on dev; branch `feat/math-p6-entry` (many commits, UNPUSHED). **NEXT SESSION (user's stated plan):** (1) **Math IPP + subject adaptations** — the Math-vs-Literacy IPP flow (creation + confirm gate) and per-subject handling; (2) **cohort data-summary pages** — a new **Math** cohort summary, **tweak the Reading** summary to "summarize all 4 items of that assessment" (clarify what the 4 items are — reading is a single level; may mean Writing's 4 traits), then **one combined RWM summary page** across all three subjects.

## Session 2026-09-04 — Fresh-laptop container setup + small-group filtering ported to Reading & Writing

Mostly an ops + one-feature session, done on a **fresh laptop** (container-only: no local node, no gh) while fine-tuning page language with colleagues in a meeting.

**Machine / container setup (all now captured in [[reference_podman_windows_dev_container]]).** Podman had no VM → `machine init`/`start`; no compose provider → plain `podman build`/`run`. Hit the Windows/podman **IPv6 gotcha**: default `-p 3000:3000` makes `wslrelay` bind `::1` only and every request empty-replies though the app is healthy inside the container; fixed by publishing `-p 127.0.0.1:PORT:3000`. `.env` files came by thumb drive (gitignored); `.env.dev` correctly targets `Assessment_Warehouse_Dev` + `AUTH_MODE=dev`. Stood `awdev` up on :3001 for the meeting (synthetic, dev impersonation bar — no live PII).

**Git flow.** Pulled main current (fast-forwarded to `8fe49be`, the 2026-09-02 wrap — AccessLevel fix live, image slimmed 585→421 MB, prod on `c30095b`). Fetch pruned six deleted remote branches. Started `session-2026-09-04`… then user redirected to pull the **most recent** branch `feat/math-p6-entry` (`9aad30f`, 2026-09-03 Math wrap) and work there. Also built a stale `assessment-webapp-f4432b6.tar` image export earlier (left untracked on the laptop; superseded — prod is on `c30095b`).

**Feature — small-group filtering on Reading & Writing entry (committed `93c8c18`, pushed to `feat/math-p6-entry`).** User asked to mirror the Math roster's small-group filter onto the Reading and Writing entry grids. Added a shared module `webapp/src/app/enter/[windowId]/[groupKey]/smallGroup.tsx` — a `useSmallGroup(roster)` hook + `SmallGroupFilter` component providing the grade-chip filter (multi-grade homerooms) + collapsible student picker ("Show just the small group you pulled to assess", Select all/Clear all), reusing the existing Math filter CSS. Wired into `RosterEntry.tsx` and `WritingRosterEntry.tsx` via `roster.filter(sg.isShown)`. **Filtering is display-only** — hidden students keep staged edits and still save (matches Math). Left the working Math component untouched (didn't refactor it onto the shared module — offered as a later step). Validated by the image build (`next build` green — no local node to run tsc). awdev rebuilt on the change (`sg-wip`).

**Feedback — "stop being a quitter."** Declared a `git push` blocked ("interactivity has been disabled") and handed it back; the fix was one retry with `GIT_TERMINAL_PROMPT=1`/`GCM_INTERACTIVE=true` and it went straight through. Reinforces [[reference_gh_cli_token_via_git]] (now generalized: attempt the obvious override before declaring blocked; the only hard stops are hanging-unwatched and credential-secret exfiltration).

**Left off:** small-group filter on Reading/Writing committed + pushed on `feat/math-p6-entry` (`93c8c18`); awdev running the change on the laptop. Branch is mid-development (Math IPP flow, cohort/RWM summary pages, DQ, FR seed still open per the 2026-09-03 note) — **no PR to main opened** (user owns that merge decision). User handing off to their desktop. NEXT unchanged from 2026-09-03: Math IPP + subject adaptations, then the cohort data-summary pages (Math cohort + Reading tweak + combined RWM).

---

## Session 2026-09-04 (later) — Per-subject cycle grade bands + cycles-list polish; window→cycle wording; ASD-STE100 user guides; dev enrollment-currency troubleshooting

Continuation on `feat/math-p6-entry` (desktop). All DEV-only; live untouched.

**Per-subject cycle grade bands (decided + built).** User set the grade split per subject: **Reading P–8, Writing P–RG, Math P–6** (MinGrade moved from `PP` to `P`). `sql/scripts/set_cycle_grade_ranges_by_subject.sql` splits existing cycle rows by `AssessmentType` (region-wide, so one UPDATE per subject catches every per-subject `DimAssessmentWindow` row incl. multi-subject `CycleGroupID` rows); ran on dev, verified 3 rows. Then rebuilt the **/cycles form** to carry grades **per subject**: `ShortCycleRow`/`ShortCycle` gain `minGrade`/`maxGrade` (data.ts); `ShortCycleInput.grades: Record<subject,{minGrade,maxGrade}>` (actions.ts writes each subject's own band); `ShortCyclesManager.tsx` renders a per-subject **Grade ranges** block that appears dynamically as each subject is checked, prefilled with `SUBJECT_DEFAULT_GRADES` (R P–8 / W P–RG / M P–6) and editable; added **`RG`** to the grade dropdown (wasn't selectable). Commit `4ba2d85`.

**Cycles-list table polish** (`9c2b849`, `9497bbb`, `87ff22f`): dropped the redundant Subjects column (Grades now lists each subject+band); stacked the dates (start over end) and the Edit/Deactivate buttons; **`table-layout: fixed`** with %-widths summing 100% so the actions column stops overflowing the card and row dividers line up; vertically centered body cells. `cycles-grid` class scopes it.

**window → cycle wording** (`ef07b78`): renamed the 5 user-visible "window" strings on the Data Entry group-select + roster pages to "cycle" (subtitle, "Back to windows"→"Back to cycles", 2 empty states). Internal identifiers (`windowId`, `AssessmentWindowID`, `TeacherWindow`, DB cols, `window.history`) left alone. Aligns with the app rename. See [[feedback_avoid_assessment_term]].

**App rename / avoid "assessment".** User: the app is the **"Short Cycles of Response App"** and "assessment" should leave user-facing text. New rule [[feedback_avoid_assessment_term]]. Applied to the guides; the only survivors are the live UI "Assessment history" heading + donut "ASSESSED" label (offered to rename in code, not yet done).

**ASD-STE100 user guides (7 one-pagers, `docs/user-guides/*.docx`).** User dropped the earlier "webpage artifact" approach — wanted **editable Word docs** in Simplified Technical English, split into one-pagers: access/sign-in, enter reading, enter writing, enter math, view cohort, view individual student, confirm IPPs (skipped cycles + ingest). Mapped exact UI labels via an Explore agent. **Word COM hung** (non-interactive-session message-pump block — simple property reads worked, document edits hung); killed it and generated the `.docx` as **Open XML directly** (zip of document.xml/styles.xml/numbering.xml via `System.IO.Compression`, generator `scratchpad/build-guides-openxml.ps1`). Real Word styles + auto-numbered steps + `[SCREENSHOT]` placeholders; Unicode (✓✗→Δ) preserved; validated well-formed. Sent to the user via SendUserFile. Then reworded to drop "assessment".

**Troubleshooting — teachers see no open cycle (root-caused + fixed).** With the bands set, only privileged accounts + one teacher saw the open cycle **SCoR 1** (2026-09-01→10-05, R P–8/W P–RG/M P–6). Walked the roster chain (`FactSectionTeachers.TeacherEmail`→`DimSection`→`FactEnrollment`→`DimStudent.Grade`) with a ladder of dev diagnostics. **Root cause: dev `FactEnrollment` is all 2025-2026 and expired** (41 rows, EndDate ≤ 2026-06-30, only 1 open-ended → 1 current on 2026-09-04). NOT a linkage/app/grade-band bug. Fix: `rollforward_enrollment_dev.sql` shifts enrollments +1 year → after it the P-6 homeroom teachers (Aurora Maple/Bryce Birch/Cedar Pine) resolve all three subjects, Elder Spruce (7-8) Reading+Writing, Daphne Oak (grade 9) Writing. Captured as [[project_dev_enrollment_year_rollover]] (recurs every school year). Diagnostic scripts kept in `sql/scripts/*_dev.sql`.

**Reading grade-8 benchmark gap (flagged, not fixed).** Reading now spans P–8 but `DimReadingBenchmark` is seeded only through Grade 7 (carry-over of the Grade-6-June expectation). `usp_UpsertReadingAssessment` **tolerates** a missing benchmark (ReadingDelta NULL + "AcceptedWithWarnings" audit), so grade-8 reading entry works but has no delta/on-level context until grade-8 rows are seeded. Open item.

**Left off:** all above committed on `feat/math-p6-entry` (5 commits `4ba2d85`→`ef07b78`) + wrap commit; **pushed**; **no PR to main** (branch still mid-development — Math IPP flow, cohort/RWM pages, DQ, FR seed open). Dev has cycles split to per-subject bands + enrollments rolled forward to 2026-2027 (teachers resolve). NEXT (user's standing plan): Math IPP + subject adaptations; cohort data-summary pages (Math cohort + Reading tweak + combined RWM); optionally seed grade-8 reading benchmarks and rename the live "Assessment history"/"ASSESSED" UI labels.

## Session 2026-09-08 — v0.3.0 hotfix shipped (homeroom `/` 404 GroupKey + filters), semver versioning started, secondary-group redesign scoped

**usp_MergeStaff stacked-marker fix → LIVE.** Session opened resolving a "two dupes in the DQ audit" report (two teacher emails). Traced to `usp_MergeStaff` stacking duplicate `IsCurrent=1` deactivation markers: phase 4c re-fired on repeat same-day ingests, and a multi-day RETURN left the marker open. A user re-ingest reproduced it AFTER my first patch (I wrongly assumed they hadn't run the new proc — they had; sharp correction taken), which proved the patch incomplete → added a `NOT EXISTS current row` guard on 4c + phase 4c-prime2. Clean ingest confirmed. **DEPLOYED LIVE.** Captured [[project_scd_staff_marker_restack_fix]].

**Homeroom `/`-in-name 404 → materialized GroupKey (the v0.3.0 headline).** Homerooms whose PS name contains `/` (e.g. `5/6`) 404'd their roster route — the `/` enters the URL path and IIS blocks `%2F` (`allowDoubleEscaping=false`), so URL-encoding can't fix it. User steered the design from a naive `/`→`-` swap to a **materialized column**: `DimStudent.GroupKey = <SchoolAbbrev>-<cleanedHomeroom>` (school abbreviation from `DimSchool`, homeroom cleaned of `/ \ space`), computed at ingest in `usp_MergeStudent`. School-qualified, so it also fixes same-name-different-school collisions. Both the group emit (`tvf_TeacherGroups`) and roster match (`tvf_TeacherRoster/Writing/Math`) read the same stored column → self-consistent; web keeps showing the real homeroom + school. Built + tested on dev (planted Homeroom `5/6` → `SRHS-5-6`, roster opened, header "Homeroom 5/6 · Shelburne Regional High School"). Fabric gotcha: `COL_LENGTH` unsupported → migrate script guards on `INFORMATION_SCHEMA.COLUMNS`.

**Ported to a main-based hotfix + two filters.** Prod is `main`-based (no Math), so the GroupKey work + a **small-group roster filter** (Reading/Writing) + a NEW **collapsible school filter** on the choose-a-group screen (mirrors the small-group picker: `mfilter` panel + checkboxes + Select all/Clear all; hidden for single-school staff) were assembled in the `../Assessment-Data-prod` worktree → **PR #28 → merged to `main`**.

**Semantic versioning adopted (user asked to start tracking versions).** This is the first patch to the second minor already on live; since it *also* ships features, we called it **v0.3.0** (minor), not 0.2.1. Started `CHANGELOG.md`, bumped `webapp/package.json` 0.1.0→0.3.0, tagged `v0.3.0`. Prod images now **semver-tagged** (`assessment-webapp:0.3.0`, tar `assessment-webapp-0.3.0.tar`) instead of SHA/`:token`; `docs/prod-container-swap.md` rewritten for the new scheme + a release-SQL-prerequisite callout. Closes the [[project_image_versioning_scheme]] TODO.

**Live SQL deployed; 404 cleared.** User ran `migrate_DimStudent_add_GroupKey.sql` → `usp_MergeStudent.sql` → `deploy_groupkey_tvfs_live.sql` on live and **confirmed the `/` 404 is gone**. Container swap to `:0.3.0` left for IT (tar handoff). **`feat` reconciled** — the school filter + deploy bundles committed there too, `package.json`→`0.4.0-dev` with an `[Unreleased]` CHANGELOG section, CHANGELOG+runbook **cherry-picked** from main (clean, no conflicts). Per user: **`feat` IS pushed to origin for BACKUP but NOT merged/PR'd to `main`** (main stays the release state; the earlier "don't push" was a push/merge terminology mix-up, corrected same session). Also discussed the reusable hotfix-off-main playbook (branch off main → PR → tag → walk the fix/metadata forward to the feature branch; cherry-pick metadata-only commits when the code already lives on the feature branch).

**Grade splits confirmed on LIVE.** Verified `DimAssessmentWindow`: Reading P-8 / Writing P-RG / Math P-6 all present on live (they *were* applied). Noted the live `/cycles` form still can't split cycles per subject — that capability is dev/`feat`-only.

**Secondary (10-RG) group display — REDESIGN SCOPED, not built.** A VP-who-teaches-50% saw a fragmented mess of HS sections (many cards, 1-5 students each). Root-caused: group resolution is a mutually-exclusive `AccessLevel` dispatch, so a dual-role Administrator NEVER gets the teacher branch — their own classes vanish into the school-wide "every section any in-scope student sits in" dump. **User REVERSED the earlier "move the split to grade 7" plan:** 7/8/9 STAY on homeroom (cohorted, travel with their homeroom). New target — **teachers**: P-9 homerooms they teach in + only the HS sections they teach; **above-teacher roles**: a **Homeroom-view ⟷ Section-view toggle** over full P-RG + a **grade filter atop the school filter**. Captured [[project_group_display_redesign]]. **Also flagged (OPEN bug):** HS **sections under-count students** — Drumlin English 10 shows 4, should be ~15 → [[project_section_undercapture_bug]]. Both to be chased next session.

**Left off:** v0.3.0 merged to `main` + tagged; live SQL done + 404 verified cleared; **container swap to `:0.3.0` pending IT** (`assessment-webapp-0.3.0.tar` ready in `C:\Git-Repos\`). `feat` at `0.4.0-dev`, pushed to origin for backup (not merged to main). NEXT SESSION: (1) diagnose the section under-count ([[project_section_undercapture_bug]]) — likely the same root as the fragmentation; (2) build the group-display redesign ([[project_group_display_redesign]]) — dual-role teacher rule + above-teacher homeroom/section toggle + grade filter. Do the diagnosis on dev/synthetic (live-PII boundary).

## Session 2026-09-09 — enrollment ActiveFlag bug (fixed both 0-active AND the section under-count), prior-year baseline loaded, display scoped

**The section under-count = the ActiveFlag bug (one root cause, two symptoms).** Set out to diagnose Drumlin's "English 10 shows 4, should be ~15." Long careful diagnosis (with a couple of my own missteps the user caught): first a two-school artifact — my discovery query used `SchoolName LIKE '%Drum%' OR Abbreviation LIKE '%DR%'`, and Digby Regional's abbrev `DRHS` matched, so a section dump looked like "duplicate sections" (it wasn't). Scoped to `SchoolID='0981'` and found: DHCS ENG10 141513 had two DimSection versions (re-versioned 09-08 on EnrollmentCount); **13 students stranded on the stale version, 4 on current** → the roster (joins current) showed 4. Also learned **Fabric LIKE is case-sensitive** (BIN2_UTF8) — `'%English%'` never matched `ENGLISH 10`. Then found the whole `FactEnrollment` table was **0 active / 39 098 inactive**. Root cause (user supplied the key fact): **PowerSchool always populates `CC.DateLeft` with the scheduled term-end**, never blank for a continuing student — so `usp_MergeEnrollment`'s old `ActiveFlag` logic (blank=active + a hardcoded Jan/June term-end month map) evaluated EVERY row to 0. Fixed to "DateLeft NULL or >= today = active." And crucially, Step 2 only re-resolves an enrollment's `SectionKey` to the current DimSection version **when the row is active** — so all-inactive froze every SectionKey → the stranding. **Fixing ActiveFlag + re-running the merge fixed BOTH**: 0→38 028 active, and Drumlin ENG10 4→16 / 0→18. My first repair attempt (a blunt EndDate-only recompute) was wrong — it ignored export presence and made everything active (39 098); the user caught it (they know the export = 38 311); the right fix was just re-running the (fixed) proc. **Deployed to live; PR #29 → main.** [[project_section_undercapture_bug]] flipped RESOLVED.

**281 orphaned enrolments = 12 blank-teacher-email sections.** The proc's standing "282 excluded — SectionID didn't resolve" warning: verified (not assumed — the user called out my assuming) that all 12 are in the Sections export, have students, but have a **blank teacher email** → `usp_MergeSection` drops them (`TeacherStaffKey NOT NULL`). User will fix the emails in PS for the next merge. New principle captured: **ingest must surface excluded rows to the operator, not drop silently** → [[feedback_ingest_surface_missing_data]] (silent skip → later "teacher can't see their students" with no breadcrumb). Also captured **Fabric gotcha: no aggregate over a subquery** (Msg 130) — compute per-row flags in a CTE then SUM.

**Prior-year baseline loaded to live ("option B").** New request: let teachers see where new students started (June 2025-2026 reading/writing). Chose the lightweight denormalized `PriorYearBaseline` (full star-schema historical DimStudent versions = "option A", deferred). Source = four ELA/FLA Reading/Writing sheets (real data can't go to dev — PII — but the table is standalone/additive/idempotent, so loaded straight to live). Read the xlsx by unzipping the OOXML in PowerShell (no Python/ImportExcel). Key modelling calls, all user-corrected into shape: grain **(StudentNumber, AssessmentLanguage)** where AssessmentLanguage = the assessment's LANGUAGE (ELA→English/FLA→French), NOT program — so immersion-in-both-languages and immersion→English transfers correctly get two rows; cross-school transfers legitimately dupe (sheets aggregated per school — verified 23/28 dupes are transfers, 3 true dupes, 3 blank-school). Writing has all 4 traits; scores VARCHAR to hold codes (`SCR` go-forward; `ABS`/`INS`/`EAL` historical; all → NULL in averages, never 0). **Fabric gotcha: `COPY INTO` path is a storage path, not a URL — literal spaces, NOT `%20`** (my `%20` gave a silent 0-row load until fixed). Loaded: 6221 rows / 5797 students / 5385 current matches. [[project_prior_year_baseline]], [[project_writing_scribed_score_code]].

**Display scoped + design locked, NOT built.** Ships **v0.4.0 minor from `feature/prior-year-baseline`** (off `main`, baseline SQL cherry-picked on), independent of feat's Math. Design locked with the user: (1) build a **starting-point read = `COALESCE(latest prior-year FactAssessment*, PriorYearBaseline seed)`** so it auto-flips to in-system facts in **Sept 2027** with zero code change (don't hardcode a PriorYearBaseline join); (2) roster row shows starting level + **cumulative Δ vs the June anchor** (reading = `DimReadingScale.LevelOrder` count e.g. `M→P +3`; writing = 4-trait avg change; codes→NULL); (3) student-detail per-cycle Δ as a fast follow.

**Left off:** ActiveFlag fix live + on `main` (PR #29). Prior-year baseline table+load DONE on live. Branch `feature/prior-year-baseline` created off `main` with the baseline SQL; **display NOT built** — that's the next task (starting-point read → reading-roster TVF + data.ts + row UI → writing → detail → 0.4.0 container to live). `feat` unchanged this session except the ActiveFlag + baseline SQL commits (pushed for backup). NEXT SESSION options: build the v0.4.0 display, OR the group-display redesign ([[project_group_display_redesign]], still open). Validate the baseline join with the Maple-Grove grade-6 query from this transcript. Also open: deploy the ActiveFlag fix to DEV; reading-vs-writing baseline coverage (~2854 vs 4939) — confirm expected.

## Session 2026-09-10/11 — v0.4.0 + v0.4.1 shipped; Programming makeover designed + Phase 0 built; BIG branch reconciliation + workflow overhaul (feat drift fixed)

**v0.4.0 reading baseline SHIPPED to live (09-10)** — Prev June / Since June / Diff-from-Prev-Cycle columns, 7-8 auto-hide of already-Meeting students, optimistic instant save. Then **v0.4.1 hotfix SHIPPED to live (09-11)**: (a) "Diff from Prev Cycle" was blank on the FIRST cycle of the year — `PrevCycleReadingLevel` (ReadingCycleRank rn=2) doesn't exist on cycle 1, so `prevOrder` now falls back to `juneOrder` (the Prev June anchor) → cycle 1 reads == Since June; (b) **version footer + "What's new" popup** (right-aligned `vX.Y.Z`, click → teacher-facing notes for the current minor line, titled with full version + "(Updated on MM/DD/YYYY)"; notes in `webapp/src/lib/patchNotes.ts`). Merged PR #31, tagged v0.4.1. First MID-DAY hotfix → motivated the pre-1.0 [[project_maintenance_mode]] design (poll-based countdown banner, staged entry lockout, quiet auto-save net, sysadmin set-window page). Also scoped this session: [[project_subject_course_dim]] (QoL, 1.0-wishlist).

**Programming (IPP + Adaptations) makeover — DESIGNED end-to-end + Phase 0 backend BUILT on dev.** Design (all user-driven, several corrections): rename IPP nav → **Programming**; **TWO rosters (IPP, Adaptations)**, each ONE grid, one row per student, **Reading|Writing|Math COLUMNS** (collapsed from an earlier 6-card idea). Adaptations = per-subject yes/no (NOT a taxonomy) — mirror IPP keyed on `DimStudent.Adap`. **Adaptive cell** handles the FI EN/FR literacy split in one column: single-literacy students + Math → No/Yes; **FI grade-3+ literacy → 4-way No / FLA-Only / ELA-Only / Both** (writes the two program-family rows). **Grade-band prefilter** (Reading P-8, Writing P-RG, Math P-6; PP excluded) — driven by seeding, so a cell shows only where the subject applies. **Non-teachers get the shared group picker FIRST** (card selection gates the rosters). Core French ('French Second Language') is NOT assessed → no rows. **Phase 0 backend (DONE + validated on dev):** Math guard relaxed in `usp_UpsertStudentIPP`; new `FactStudentAdaptation`; `usp_MergeStudent` Step 6 rewrite (Math IPP P-6 own-family single row + per-subject bands + parallel Adaptation 6c/6d); `usp_UpsertStudentAdaptation` (write, granted) + `tvf_StudentAdaptation` (@UPN read). Seeding PROVEN on dev: Math/FI IPP rows created + grade-9+ English get Writing-not-Reading (bands work). Full spec [[project_programming_makeover]]. **Remaining: Phase 1 shared picker + Phase 2 rosters (frontend).**

**BIG: branch reconciliation + workflow overhaul.** Discovered `feat/math-p6-entry` had drifted **29 commits behind `main`** (diverged 09-02, before v0.3.0) AND carried its OWN divergent GroupKey/school-filter work — while I had (wrongly) been putting the makeover on `feature/prior-year-baseline` and reporting "merged main in" about THAT branch, not feat. The user (rightly) called this out — a real trust hit; owned it with the git facts. Fix: cut a fresh **`dev`** off current `main`, merged `feat` in (7 conflicts resolved to main for shared infra; one semantic break — dropped `revalidatePath` import for the math save — fixed; `next build` verified green), then **deleted `feat/math-p6-entry` AND `feature/prior-year-baseline`** and the stale `hotfix/*`. Enabled GitHub **auto-delete-head-branch-on-merge**. New WORKFLOW set → [[feedback_git_workflow]]: **dev worktree** (`Assessment-Data`, long-lived `dev`) + **patch worktree** (`Assessment-Data-prod`, on `main`; cut `hotfix`/`critical-patch` off main); **MANDATORY `main`→`dev` back-merge after every patch** (the missing step behind the drift); consistent names only; always name WHICH branch. Clean two-branch state now: `main` (v0.4.1 live) + `dev` (0.5.0-dev). (Worktree dir rename `-prod`→`-patch` blocked by a Windows lock — cosmetic, pending.)

**Errors / lessons this session (mine):** (1) reported "merged main in" about `feature/prior-year-baseline` while the user meant feat — branch-naming ambiguity → the drift went unseen; fixed by the consistent-naming workflow. (2) Told the user to "re-run the merge / `usp_RunFullIngestCycle`" to seed programming rows on dev — WRONG: the orchestrator has no transaction, so its merges commit before the DQ check, and Steps 1-5 reconcile DimStudent from staging and **close any current student absent from staging** → it deactivated the directly-inserted Drumlin roster. The DQ halt (Msg 51000) was actually 7 PRE-EXISTING orphaned FactAssessmentReading/Writing rows (stale window IDs), unrelated to my changes. Recovery: delete the orphans + rebuild Drumlin. Correct tool built: `reseed_programming_rows_dev.sql` (Step-6-only; never touches DimStudent). Lesson: never full-ingest to seed on dev; the ingest cycle is not transactional.

**Dev tooling:** `seed_primary_homeroom_dev.sql` — 20-student Primary homeroom at **Drumlin (0981, DHCS-PA)** wired to a generated synthetic teacher (`devseed.teacher@tcrce.ca`), for viewing the math matrix with a larger roster. `reseed_programming_rows_dev.sql` — the safe Step-6 reseed.

**In-flight (paused): TD/DT bug.** FI P/1 homeroom shows Expected mixing `TD` (French scale) and `DT` (English scale) — ONLY grade-Primary; grade 1 all `2–3` (French). Diagnosed via the deterministic benchmark join (`drb.ProgramFamily = sg.ProgramFamily`): mixed Expected in one grade ⟹ mixed PROGRAMS, so the `DT` grade-P students are on the **English program** (mislabeled immersion, or a genuinely mixed class). User is verifying the PS file's ProgramCodes (+ remember: warehouse only reflects a code correction after a re-ingest). **Related real bug flagged:** the roster's New-level **dropdown uses ONE scale for the whole grid** (`page.tsx` fetches `getScaleLevels(roster[0].scaleSystem)`), so mixed-program homerooms show the wrong-scale dropdown to the minority — worth a separate fix.

**Left off:** `main` = v0.4.1 (live). `dev` = 0.5.0-dev, Phase 0 backend committed + pushed. NEXT: (1) resolve the TD/DT question once the user confirms the ProgramCodes (data vs. app); consider fixing the per-grid dropdown-scale bug; (2) resume the makeover — Phase 1 shared group picker ([[project_group_display_redesign]]), then Phase 2 rosters. Deploy list for the makeover backend to dev/live: `FactStudentAdaptation.sql` → `usp_MergeStudent.sql` → `usp_UpsertStudentAdaptation.sql` → `tvf_StudentAdaptation.sql` → re-run `grant_webapp_sp.sql` (first two already on dev). Blockers: none.

## Session 2026-09-15 — Programming makeover Phase 1 COMPLETE (shared picker + roster resolution + grade-span + Grade lens); dark-mode wishlist; MEMORY.md compaction

Continued the makeover on `dev` (0.5.0-dev). Everything below is committed on `dev`; SQL deployed to the DEV warehouse by the user as we went; web changes built into `awdev` (assessment-webapp:0.5.0-dev). No live/main changes.

**Shipped this session (all on dev):**
1. **Cohort filter persistence** — `/students` `CohortView.tsx` now persists its filters (grade bounds, gender, self-ID, homeroom/program/school/achievement, expanded) for the tab session via sessionStorage (default → mount-effect restore → `ready`-gated persist; Set filters intersect saved w/ current options, empty = show all; grade bounds clamp into range). Same SSR-safe pattern as the picker. Also moved the oversight grade **Select all/Clear all** inline at the end of the chip row (`.chip-actions` margin-left:auto) to reclaim a row.
2. **Roster resolution now lens-agnostic** — `tvf_TeacherRoster` + `tvf_TeacherRosterWriting` `StudentGroups` were a single CASE (homeroom for PP-9, section for HS), so an oversight HS **Homeroom** card resolved to an EMPTY roster. Rewrote to emit a homeroom candidate key AND (HS) a section key per student; only the row matching `@GroupKey` survives the final WHERE, SELECT DISTINCT collapses. Deployed to dev + verified.
3. **Grade filter is grade-span aware** — the oversight grade filter tagged each card by `MAX(Grade)`, so a split P/1 homeroom only surfaced under Primary and vanished when filtering to Grade 1. `tvf_TeacherGroups` now emits a `Grades` list per group (GroupGrades CTE + `STRING_AGG` over distinct grades); client parses to `grades[]` and shows a card if ANY of its grades is selected (handles N-grade splits). Test fixture `seed_split_homeroom_dev.sql` re-grades the DEVSEED Drumlin homeroom to **P/1/2**. Verified on dev.
4. **Regenerated `deploy_groupkey_tvfs.sql`** — the bundle had drifted to the pre-Phase-1 picker; regenerated as a verbatim concat of the four current `sql/security` TVFs (kept in sync twice more as the TVFs changed).
5. **DECISION — oversight Homeroom lens KEEPS full P-RG** (user chose to keep both lenses over the whole range; HS appears under Homeroom AND Section; do NOT clamp at grade 9).
6. **Oversight Grade lens (new third grouping)** — user asked for "an option above the cards to select a full grade level." `tvf_TeacherGroups` now emits one Oversight `Grade` card per **(school, grade)** with key `GRADE:<SchoolID>:<Grade>` — PER-SCHOOL so the existing school filter carries it with no URL params (DECISION: **respect the school filter**; no single cross-school combined grade roster). Respects the window program family like the other lenses. All three roster TVFs thread `SchoolID` through and resolve the `GRADE:` key to the whole cohort (Math converted to the same multi-candidate `StudentGroups`; Math is P-6 so its section candidate never fires). Client `GroupCards`: `[Homerooms | Sections | Grades]` tab (each shown only when it has cards); grade cards respect the school filter; the grade-chip filter is hidden AND not applied on the Grade lens (the cards ARE the grades); roster page header labels a grade cohort ("Primary"/"Grade 3"/"Graduating · School") instead of a stray homeroom name. DECISION: available in **BOTH Data Entry and Programming** (Programming inherits it via the shared picker in Phase 2).

**Phase 1 is now COMPLETE** (shared picker + lens-agnostic roster resolution + grade-span filter + Grade lens), all verified on dev. Remaining makeover work: **Phase 2** — the Programming pages themselves (rename `/ipp`→`/programming`; two IPP/Adaptations rosters as student×Reading|Writing|Math grids with the adaptive cell, reusing the picker).

**Also:** recorded **dark mode** as a POST-1.0 QoL wishlist item ([[project_dark_mode]]) with full scoping (token foundation exists; the work is a 2nd palette + a hardcoded-color audit — ~20 literal `#fff` + ~30 semantic tints bypass the tokens — + a trigger; DB-driven `DimAchievementLevel` Hex/HexColorTint is the one design question; ~½ day system-only, ~1 day with a toggle). **Compacted `MEMORY.md`** (a PostToolUse hook flagged it near the 24.4KB read limit): ~22KB → 11.3KB, one terse line per entry across all 71 memory files, grouped into sections; full detail stays in the topic files.

**Left off:** `main` = v0.4.1 (live, untouched). `dev` = 0.5.0-dev with Phase 0 + **Phase 1 complete**, all committed + pushed at wrap. Grade picker confirmed working by the user. NEXT session: **Phase 2 Programming pages**. The makeover backend deploy-to-LIVE list still pending for when 0.5.0 ships: `FactStudentAdaptation.sql` → `usp_MergeStudent.sql` → `usp_UpsertStudentAdaptation.sql` → `tvf_StudentAdaptation.sql` → re-run `grant_webapp_sp.sql`; plus the four TVFs (`deploy_groupkey_tvfs.sql`). Blockers: none.

## Session 2026-09-16 — Programming makeover Phase 2 complete + Maintenance mode built; backlog reconciled; nav first-load fix

Continued on `dev`/0.5.0-dev. All committed on `dev`; SQL deployed to the DEV warehouse; web in `awdev`. No live/main changes.

**Backlog reconciliation (user asked to catch stale items in the polish pass).** Verified against code, corrected the decision record: (1) **Grade-8 reading benchmarks RESOLVED** — `seed_DimReadingBenchmark_grade8.sql` (v0.4.0, 2026-09-10) copies Grade-7 (=Grade-6-June carry-over) into Grade 8; verified 20 rows each for 6/7/8 on dev. (2) **HS-teachers-see-own-courses BUILT** — Phase 1 Taught scope is `FactSectionTeachers`-based (sections you teach, NOT every section a student sits in); pure teachers were always fine, the teaching-admin dual-role case is the fix (dev only). (3) **Writing `SCR` (Scribed)** — the one the user thought was done is only DECIDED (treatment recorded in [[project_writing_scribed_score_code]]; the PriorYearBaseline table tolerates codes) but NOT built in the writing entry/proc/calc path. Still the remaining unbuilt backlog item.

**Programming makeover Phase 2 — DONE (feature-complete on dev).** 2a: `/ipp`→`/programming` (git-mv route dir, redirect kept, nav/home/title renamed, middleware). 2b: NON-teacher scope = **card picker** (user chose over inline filters). Two NEW window-less TVFs — `tvf_ProgrammingGroups(@UPN)` (Taught/Oversight groups over FLAGGED students; count=needs-IPP-confirmation) + `tvf_ProgrammingRoster(@UPN,@GroupKey)` (one group's students, 1 row per (subject,family) in EITHER fact + IsIPP/HasAdaptation + existence flags; group membership by key-shape ∩ caller scope). `getProgrammingGroups` reuses `TeacherGroup`→`GroupCards`. `ProgrammingRosterGrid`: IPP⟷Adaptations toggle, student×Reading|Writing|Math, **adaptive cell driven by the DATA** (1 family row→2-way; 2 rows EN+FI→4-way No/FLA/ELA/Both — so FLA shows only for FI grade-3+ literacy, matching the seeding). Save via `saveProgramming` (usp_UpsertStudentIPP/Adaptation), scope+existence gated. 2c: single-group auto-skip; amber needs-confirmation cue + pill. Dev counts checked: 15 FI (10 grade-3+), 14 flagged appear in Programming. Deploy: tvf_ProgrammingGroups.sql + tvf_ProgrammingRoster.sql.

**Maintenance mode — BUILT (all 5 spec parts) + refined under testing.** Store = warehouse row `AppMaintenance` + `usp_Set/ClearMaintenanceWindow` (sysadmin-gated) + `/api/status` (server clock, cache, fails open, auto-expire). Client `MaintenanceProvider` (AppShell) polls → staged banner (warn→T-5 lock-after-next-save→T-3 full lock→T-1 quiet auto-save→T "we'll be right back" overlay); grids reading/writing/math/Programming lock via `useEntryLock` (writing auto-saves complete rows only). Sysadmin `/admin/maintenance` page + nav. **Refinements from live testing:** sticky lock across navigation (sessionStorage keyed by T); down = fixed overlay ON TOP (not replacing children — suspected the ~T-1 client crash); one-click **Clear** on banner + overlay; **AuthArea sign-in/out on the overlay** so a locked-out/wrong-account sysadmin can switch; dev impersonation bar moved above the overlay; poll cadence tightened (status cache 4s, client 8s/4s); banner copy softened (user "too aggressive" — kept timing, gentler words, T-5 amber). **Two Fabric gotchas hit + fixed + skilled:** no `TINYINT` (Msg 24574 → INT); bare `IF`-body `;THROW` needs `BEGIN…END` (Msg 102, already in the skill). Cleared a stuck dev window via a container exec calling `usp_ClearMaintenanceWindow`.

**Nav "basic options until refresh" fix.** Root cause: AppShell resolved auth + caps in ONE try/catch, so a cold-pool caps-query blip on first hit nuked both. Fix: `getCallerCapabilities` retries the transient server-side (3× backoff); AppShell resolves auth and caps separately (a caps error no longer un-auths); `PostLoginRefresh` gains a bounded caps-error client refresh in both modes.

**Left off:** `main`=v0.4.1 (live, untouched). `dev`=0.5.0-dev with **Programming makeover complete through Phase 2 + Maintenance mode**, all committed + pushed at wrap; SQL on dev. NEXT: build **Writing `SCR`** (scope first — touches the live writing path) OR keep polishing. 0.5.0 release (dev→main) will bundle: baseline+footer, Math P-6, Programming makeover, Grade lens, Maintenance mode. Deploy-to-LIVE SQL list for 0.5.0: Programming Phase 0 (FactStudentAdaptation → usp_MergeStudent → usp_UpsertStudentAdaptation → tvf_StudentAdaptation → grant), the group/roster TVFs (`deploy_groupkey_tvfs.sql` + `tvf_ProgrammingGroups/Roster.sql`), and maintenance (`AppMaintenance.sql` + the 2 procs). Blockers: none.

## Session 2026-09-17 — SCR shipped; dual-language + cycle-scoping + cycle header built; course-scoped entry designed (rework pending)

Very long session (continued from 09-15/16). Landed on dev/0.5.0, all committed on `dev` (not pushed to main — release-only):

**Writing `SCR` (Scribed) — DONE + verified on dev.** Scope narrowed by user mid-build to **Conventions ONLY** (not Organization). `ConventionsScore` INT→VARCHAR via multi-step migration (add temp → copy → drop → re-add same name → copy back). Omitted from the average (sum/count over scored traits, SCR→NULL via TRY_CAST). Proc validates `'1'`–`'4'`/`'SCR'`; 3 writing reads recompute avg; UI "Scr" option on Conventions only. Deploy chain ran clean on dev; user confirmed saving + avg correct.

**J020 (late immersion) reads English (reading).** `usp_MergeStudent` Step 6: drop the `(Reading,'French Immersion')` IPP/Adaptation seed for J020 (×4 CTEs); `remediate_J020_reading_english_only.sql`; `tvf_TeacherRoster` routes J020→EN_Reading + English benchmark/IPP family. Reading is single-language per student.

**Dual-language WRITING (result-level).** `FactAssessmentWriting.AssessmentLanguage` (backfilled by program family, GO-separated — hit Msg 207 first, same ADD-COLUMN-then-reference batch rule); `usp_UpsertWritingAssessment` +@AssessmentLanguage (grain, 51019 validation); `tvf_TeacherRosterWriting` +@Language. Built an interim EN/FR **toggle** on the writing entry roster (`?lang=`).

**App-level per-cycle scoping (BIG).** After several of MY wrong turns (hardcoded "FI grade 3+", "reading single-language" — user firmly corrected: assessment methodology is config, not code), landed: `DimAssessmentWindow` + `AssessmentLanguage` (EN/FR/NULL=Both) + `ProgramScope` (comma bucket set); `DimProgram.ScopeBucket` (English / Early Immersion / Late Immersion — factual, non-immersion incl. FSL→English); `usp_UpsertShortCycle` takes them; both rosters filter by `ProgramScope` (delimiter-guarded LIKE) + language; `/cycles` gained a program-scope multi-select + language dropdown. Only hardcoded rule: French literacy = French Immersion.

**Cycle HEADER model.** User: header table with a distinct key so "SCoR 1" each year stays separate for roll-up. Built `DimShortCycle` (key=CycleGroupID) + `usp_UpsertShortCycleHeader` (propagates name/dates to instance windows) + `backfill_DimShortCycle_headers.sql`. `/cycles` rebuilt: **two-part** — create cycle header (name+dates) → **instance builder** (add scoped rows: Subject × Language × ProgramScope × grades). `getShortCycles` now header+instances. All deployed on dev + image swapped; user built an 8-instance "SCoR 1" successfully (verified via container SP query).

**Entry picker "unhelpful cards" → design pivot.** User saw many identical "SCoR 1" cards on `/enter`. Diagnosed (SP container query) that all 8 instances exist correctly — the picker just couldn't distinguish them. User steered to the real model: **pick the CYCLE, not the instance**; and **tie language to the COURSE**, dropping the toggle entirely. Confirmed design (in [[project_assessment_language_tracks]]): `/enter` = one card per (SCoR × subject); group picker = the caller's **mapped-course sections** only, **grouped by language heading**, **same-language multi-select**; combined roster across sections with **per-student instance routing on save**; oversight roles lose BROAD entry (view stays broad).

**Course list received.** Saved `docs/course-assessment-mapping.md` (ELA+ImmELA→English, FLA→French, Math→Math; a LA course = Reading+Writing). Built `DimCourseAssessment` map table (data-driven, add/remove=row) + `seed_..._live.sql` (real codes) + `seed_..._dev.sql` (dev's synthetic codes — dev has NO real codes / no English LA, so dev tests French+Math only; swap live seed on deploy). Chain: teacher→FactSectionTeachers→SectionID→DimSection.CourseCode→map.

**Also:** version footer from package.json + What's-new spanning prev minor; card-label CONCAT space fix; CHANGELOG-as-we-go rule. Container **diagnostics via SP** (mssql + ClientSecretCredential, `podman exec ... --require ./load-secrets.cjs /tmp/diag.cjs`) used repeatedly to inspect DimAssessmentWindow / DimSection / DimProgram.
