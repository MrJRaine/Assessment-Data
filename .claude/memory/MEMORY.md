...
- [Math Split-Grade Pacing Model](project_math_split_pacing_model.md) — DESIGN converged 2026-09-21 (not built): default placement on task + composition exceptions resolved as effective placement; binned by MONTH (established design; guides authored in weeks, translated to match the benchmark); surrogate keys only; P-6 math roster by homeroom. Carry-forward/linking = next design piece.
