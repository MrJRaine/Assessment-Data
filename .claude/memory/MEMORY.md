...
- [Math Split-Grade Pacing Model](project_math_split_pacing_model.md) — DESIGN converged 2026-09-21 (not built): default placement on task + composition exceptions resolved as effective placement; pacing authored in school-year WEEKS (translate→month→SCoR); surrogate keys only; P-6 math roster by homeroom. Carry-forward/linking = next design piece.
