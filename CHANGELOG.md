...
Staged on the `dev` branch, not yet released to live. Headline: **Math P–6 task-based
short cycles** (binary can-do/cannot mastery; by-task proportion + by-student 4-tier
achievement level; data layer + entry matrix) plus the in-progress **Programming
(IPP + Adaptations) makeover**.
